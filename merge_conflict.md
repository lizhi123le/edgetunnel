# 合并冲突报告
## 冲突时间: Sat Jul 11 18:30:47 UTC 2026
## 上游更新哈希: f9df5455cb4056e3b41fcf68f3bdd8eb459553922e253207742f1e4606e1c736

以下文件包含冲突标记，需要手动解决：

```
<<<<<<< local_worker.js
const Version = '2026-06-17 01:41:21(Enhanced)';
...
// ★ 非管理员/非随机路径请求速率限制（基于 CF-Connecting-IP，每 IP 每小时最多 12 次）
const 速率限制池 = new Map();
...
function 检查速率限制(客户端IP) {
    ...
    if (记录.时间戳队列.length >= 12) return false;
    return true;
}
...
async function 记录违规并检查拉黑(客户端IP, env, ctx) {
    ...
    try {
        ...
        ctx.waitUntil(env.KV.put(key, String(当前计数), { expirationTtl: 86400 * 30 }));
        if (当前计数 >= 永久黑名单阈值) {
            const 黑名单 = await 加载永久黑名单(env);
            if (!黑名单.has(客户端IP)) {
                黑名单.add(客户端IP);
...
const apiValue = 'sub://' + subMatch[1].trim() + (优选IP作为反代IP ? '?' + 特征码字典[0].toLowerCase() + '=true' : "") + (元素.includes('#') ? ('#' + 元素.split('#')[1]) : '');
        ...
        // ★ 速率限制 + 永久黑名单：非管理路径的请求段必须在常用路径目录中，否则限流（每 IP 每小时最多 12 次）
...
async function 请求日志记录(env, request, 访问IP, 请求类型 = "Get_SUB", config_JSON, 是否写入KV日志 = true) {
    try {
        ...
        const 日志内容 = { TYPE: 请求类型, IP: 日志IP, ASN: `AS${request.cf?.asn || '0'} ${request.cf?.asOrganization || 'Unknown'}`, CC: 日志CC, URL: sanitizeURL(request.url), UA: sanitizedUA, TIME: 当前时间.getTime() };
