# 合并冲突报告
## 冲突时间: Fri Apr 10 14:07:50 UTC 2026
## 上游更新哈希: 04601ad67a2b8d77d588079028dc2cfa06606a45c520ba68653cf49706410d5e

以下文件包含冲突标记，需要手动解决：

```
<<<<<<< local_worker.js
const Version = '2026-04-10 06:03:17 (Local Enhanced)';
=======
                ...
                if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
                    ...
                    if (url.searchParams.get('token') === 订阅TOKEN || 作为优选订阅生成器) {
                        ...
                        if (订阅类型 === 'mixed') {
                            ...
                            订阅内容 = 其他节点LINK + 完整优选IP.map(ip => IP转换节点(ip)).filter(item => item !== null).join('\n');
                        } else { // 订阅转换
			...
			ss初始化任务 = (async () => {
				...
				const 入站解密器 = {
					async 输入(dataChunk) {
						...
						while (true) {
							入站状态.cpuYieldCounter++;
							if (入站状态.cpuYieldCounter >= 8) {
								入站状态.cpuYieldCounter = 0;
