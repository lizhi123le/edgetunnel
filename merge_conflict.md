# 合并冲突报告
## 冲突时间: Sun May 17 02:03:13 UTC 2026
## 上游更新哈希: 22619b86579feae043ae098b1a9d1868c156b77a2c9e17a1886ab5b33b101beb

以下文件包含冲突标记，需要手动解决：

```
const Version = '2026-05-15 18:51:29(Enhanced)';
...
async function generateCookieSignature(cookieValue, secret) {
    const key = await crypto.subtle.importKey(
        'raw',
        new TextEncoder().encode(HMAC_SIGN_KEY_PREFIX + secret),
        { name: 'HMAC', hash: 'SHA-256' },
        false,
        ['sign']
    );
    const signature = await crypto.subtle.sign('HMAC', key, new TextEncoder().encode(cookieValue));
...
export default {
    async fetch(request, env, ctx) {
        ...
        反代IP = ctx2.反代IP; 启用反代兜底 = ctx2.启用反代兜底;
        ...
        if (env.GO2SOCKS5) {
            ...
            SOCKS5白名单 = ctx2.SOCKS5白名单;
            // 同步更新预编译正则
            ctx2.SOCKS5白名单正则 = SOCKS5白名单.map(p => new RegExp(`^${p.replace(/\*/g, '.*')}$`, 'i'));
            SOCKS5白名单正则 = ctx2.SOCKS5白名单正则;
        }
        if (访问路径 === 'version' && url.searchParams.get('uuid') === userID) {// 版本信息接口
            return new Response(JSON.stringify({ Version: Number(String(Version).replace(/\D+/g, '')) }), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
        }
        if (管理员密码 && upgradeHeader === 'websocket') {// WebSocket代理
            await 反代参数获取(url, userID);
            ctx2.syncFrom({ 反代IP, 启用反代兜底, 启用SOCKS5反代, 启用SOCKS5全局反代, 我的SOCKS5账号, parsedSocks5Address });
            log(`[WebSocket] 命中请求: ${url.pathname}${url.search}`);
            ...
            if (env.KV && typeof env.KV.get === 'function') {
                ...
                if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
                    ...
                    if (request.method === 'POST') {
                        ...
                        if (Number(登录尝试次数) >= 5) return sanitizedErrorResponse('登录尝试次数过多，请10分钟后再试', 429);
                        const formData = await request.text();
                        const params = new URLSearchParams(formData);
                        const 输入密码 = params.get('password');
                        // 使用 timingSafeEqual 防时序攻击
    ...
    return new Response(new ReadableStream({
        async start(controller) {
            ...
            const 写入远端 = (payload, allowRetry = true) => {
                ...
                return XHTTP上行写入队列.写入(payload, allowRetry);
            };
...
async function 处理WS请求(request, yourUUID, url, ctx2) {
	...
	const 处理WS入站数据 = async (chunk) => {
		let 当前块字节 = null;
		if (isDnsQuery) {
			if (判断是否是木马) return await 转发木马UDP数据(chunk, serverSock, 木马UDP上下文, ctx2);
			return await forwardataudp(chunk, serverSock, null, null, ctx2);
		}
		if (判断协议类型 === 'ss') {
			await 处理SS数据(chunk);
			return;
...
async function ChaCha20Block(key, counter, nonce) {
        ...
        const chachaYield = createYieldStrategy(2);
...
async function ChaCha20XOR(key, nonce, data) {
        ...
        async function Poly1305Mac(key, message) {
            ...
            const polyYield = createYieldStrategy(3);
...
async function sstpConnect(proxy, targetHost, targetPort, TCP连接 = null) {
	...
	try {
		...
		socket = (TCP连接 || connect)({ hostname: serverHost, port: serverPort }, { secureTransport: 'on', allowHalfOpen: false });
...
async function httpsConnect(targetHost, targetPort, initialData, TCP连接 = null) {
	const { username, password, hostname, port } = parsedSocks5Address;
...
async function httpConnect(targetHost, targetPort, initialData, HTTPS代理 = false, TCP连接 = null) {
    const { username, password, hostname, port } = parsedSocks5Address;
    const socket = HTTPS代理
        ? (TCP连接 || connect)({ hostname, port }, { secureTransport: 'on', allowHalfOpen: false })
        : (TCP连接 || connect)({ hostname, port });
...
// AES-128-CBC 加密函数 (用于订阅内容加密)
async function AESEncrypt(plaintext, password) {
    if (!plaintext || !password) return null;
    try {
        const keyHash = await crypto.subtle.digest('MD5', SS文本编码器.encode(password));
...
async function SOCKS5可用性验证(代理协议 = 'socks5', 代理参数, request = null) {
    ...
    try {
        ...
        if (代理协议 === 'socks5') {
            ...
            tcpSocket = 代理协议 === 'https' && isIPHostname(hostname)
                ? await httpsConnect('check.socks5.090227.xyz', 80, initialData, TCP连接)
                : await httpConnect('check.socks5.090227.xyz', 80, initialData, 代理协议 === 'https', TCP连接);
        }
