# 合并冲突报告
## 冲突时间: Mon May 11 00:21:26 UTC 2026
## 上游更新哈希: b61e94281e47a472ea8356eb276e08df0ccf45188f363f095c8587d207c142f5

以下文件包含冲突标记，需要手动解决：

```
<<<<<<< local_worker.js
const Version = '2026-05-08 19:07:28(Enhanced)';
=======
...
const 节点黑名单 = new Map(); // 缓存失效节点，避免短期内重复尝试导致 loadShed
...
const CONFIG_CACHE_TTL = 60 * 60 * 1000; // 1小时缓存，减少 CPU 负载和 KV 读取频率
            ...
            if (env.KV && typeof env.KV.get === 'function') {
                ...
                if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
                    ...
                    if (request.method === 'POST') {
                        ...
                        const 输入密码 = params.get('password');
                        // 使用 timingSafeEqual 防时序攻击
                        if (输入密码 && secureCompare(输入密码, 管理员密码)) {
                            // 清除失败计数
                    ...
                    if (访问路径 === 'admin/log.json') {// 读取日志内容
                        ...
                        const 代理协议 = ['socks5', 'http', 'https', 'turn', 'sstp'].find(类型 => url.searchParams.has(类型)) || null;
                        if (!代理协议) return new Response(JSON.stringify({ error: '缺少代理参数' }), { status: 400, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                        const 检测代理响应 = await SOCKS5可用性验证(代理协议, url.searchParams.get(代理协议));
                        return new Response(JSON.stringify(检测代理响应, null, 2), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                    }
...
async function 读取XHTTP首包(reader, token) {
    const 尝试解析魏烈思首包 = (data) => {
        ...
        if (cmd !== 1 && cmd !== 2 && cmd !== 3) return { 状态: 'invalid' };
                    ...
                    while (pending.byteLength >= 5) {
                        ...
                        if (remoteConnWrapper.socket) {
                            ...
                            if (判断是否是木马) {
                                ...
                                //console.log(`[gRPC] 木马首包: ${hostname}:${port} | UDP: ${isUDP ? '是' : '否'}`);
                                if (isSpeedTestSite(hostname)) throw new Error('Speedtest site is blocked');
                                if (isUDP) {
                                    if (port !== 53) throw new Error('UDP is not supported');
                                    isDnsQuery = true;
                                    await 转发木马UDP数据(rawClientData, grpcBridge, 木马UDP上下文, ctx2);
                                } else {
...
async function 处理WS请求(request, yourUUID, url, ctx2) {
	...
	readable.pipeTo(new WritableStream({
		...
		if (判断协议类型 === null) {
			...
			const isVless = bytes.byteLength >= 17 && formatIdentifier(bytes.subarray(1, 17)) === yourUUID;
			const isTrojan = bytes.byteLength >= 58 && bytes[56] === 0x0d && bytes[57] === 0x0a;
			if (isVless) {
				判断协议类型 = '魏烈思';
			} else if (isTrojan) {
				判断协议类型 = '木马';
			} else if (url.searchParams.get('enc')) {
				判断协议类型 = 'ss';
			} else {
				判断协议类型 = '魏烈思';
			}
		...
		if (判断协议类型 === 'ss') {
			...
			if (判断协议类型 === '木马') {
				...
				if (isUDP) {
					isDnsQuery = true;
					if (有效数据长度(rawClientData) > 0) return await 转发木马UDP数据(rawClientData, serverSock, 木马UDP上下文, ctx2);
					return;
				}
...
function 解析木马请求(buffer, passwordPlainText) {
    ...
    if (cmd !== 1 && cmd !== 3) return { hasError: true, message: "unsupported command, only TCP and UDP are allowed" };
...
async function forwardataTCP(host, portNum, rawData, ws, respHeader, remoteConnWrapper, yourUUID, ctx2) {
    ...
    const 连接超时毫秒 = 2000; // 缩短超时时间，防止死 IP 长期占用 Worker 资源
    let 已通过代理发送首包 = false;
    ...
    async function connectDirect(address, port, data = null, 所有反代数组 = null, 反代兜底 = true) {
        ...
        if (所有反代数组 && 所有反代数组.length > 0) {
            const 最大重试深度 = 5; // 限制单次请求尝试的节点数量
            let 已尝试次数 = 0;
            for (let i = 0; i < 所有反代数组.length && 已尝试次数 < 最大重试深度; i++) {
                ...
                // 自动剔除机制：检查是否在临时黑名单中
                if (节点黑名单.has(节点标识)) {
                    if (Date.now() < 节点黑名单.get(节点标识)) continue;
                    else 节点黑名单.delete(节点标识);
                }

                已尝试次数++;
                try {
                    log(`[反代连接] 尝试连接到: ${反代地址}:${反代端口} (尝试次数: ${已尝试次数}/${最大重试深度})`);
                    ...
                    节点黑名单.delete(节点标识); // 连接成功，确保从黑名单移除
                    缓存反代数组索引 = 反代数组索引;
                    return remoteSock;
                } catch (err) {
                    log(`[反代连接] 连接失败: ${反代地址}:${反代端口}, 错误: ${err.message}`);
                    // 记录失败，拉黑 3 分钟
                    节点黑名单.set(节点标识, Date.now() + 3 * 60 * 1000);
...
async function connectStreams(remoteSocket, webSocket, headerData, retryFunc) {
	...
	const BYOB缓冲区大小 = 128 * 1024, BYOB单次读取上限 = 64 * 1024, BYOB高吞吐阈值 = 50 * 1024 * 1024; // 降低缓冲区大小以节省内存，防止 loadShed
...
async function Clash订阅配置文件热补丁(Clash_原始订阅内容, uuid = null, ECH启用 = false, HOSTS = [], ECH_SNI = null, ECH_DNS, gRPCUserAgent = null) {
    ...
    let patchYieldCount = 0;
    const dynamicYield = async () => { if (++patchYieldCount % 300 === 0) await new Promise(r => setTimeout(r, 0)); };
...
async function Surge订阅配置文件热补丁(content, url, config_JSON) {
    ...
    for (let x of 每行内容) {
        ...
        if (++yieldCount % 300 === 0) await new Promise(r => setTimeout(r, 0));
    }
...
async function SOCKS5可用性验证(代理协议 = 'socks5', 代理参数) {
    ...
    try {
        ...
        let tcpSocket;
        if (代理协议 === 'socks5') {
            tcpSocket = await socks5Connect('check.socks5.090227.xyz', 80, initialData);
        } else if (代理协议 === 'turn') {
            tcpSocket = await turnConnect(parsedSocks5Address, 'check.socks5.090227.xyz', 80);
        } else if (代理协议 === 'sstp') {
            tcpSocket = await sstpConnect(parsedSocks5Address, 'check.socks5.090227.xyz', 80);
        } else {
            tcpSocket = 代理协议 === 'https' && isIPHostname(hostname)
                ? await httpsConnect('check.socks5.090227.xyz', 80, initialData)
                : await httpConnect('check.socks5.090227.xyz', 80, initialData, 代理协议 === 'https');
        }
