# 合并冲突报告
## 冲突时间: Sun May  3 05:22:18 UTC 2026
## 上游更新哈希: bee865cc7598dd8b54efccb790f1a41c8f3dbab939b5ce09e416dff46819ed60

以下文件包含冲突标记，需要手动解决：

```
const Version = '2026-05-03 01:19:25 (Local Enhanced)';
...
export default {
    async fetch(request, env, ctx) {
        ...
        调试日志打印 = ctx2.调试日志打印; // 同步到全局（兼容旧代码）
...
async function 请求日志记录(env, request, 访问IP, 请求类型 = "Get_SUB", config_JSON, 是否写入KV日志 = true) {
    try {
        const 当前时间 = new Date();
        // 日志脱敏：URL 仅记录 pathname，UA 限制 200 字符
        ...
        const KV日志条数限制 = 1000; // 改用条数限制，更可靠
        if (现有日志) {
            try {
                ...
                if (!Array.isArray(日志数组)) { 日志数组 = [日志内容]; }
                    ...
                    const sanitizedRequestURL = sanitizeURL(request.url);
                    if (日志数组.some(log => log.TYPE !== "Get_SUB" && log.IP === 访问IP && log.URL === sanitizedRequestURL && log.TIME >= 三十分钟前时间戳)) return;
                    日志数组.push(日志内容);
                    // 改用条数限制代替字节大小计算
