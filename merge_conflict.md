# 合并冲突报告
## 冲突时间: Thu Sep 24 08:53:31 UTC 2026
## 上游更新哈希: dc7428d0651f7b0773f96c87b5b764c7dcd86e5fa1f7b326fae65c8cc27cf6b5

以下文件包含冲突标记，需要手动解决：

```
<<<<<<< local_worker.js
const Version = '2026-09-22 20:01:17(Enhanced)';
...
// ★ 汇聚订阅统一 User-Agent（上游 1e2cb0c..main 新增，统一订阅请求行为）
const 汇聚订阅_UA = 'v2rayN/edge' + 'tunnel (https://github.com/' + 特征码字典[1] + '/edge' + 'tunnel)';
...
// ★ 伪装图片缓存（最多12张，每张不超过2048KB，FIFO淘汰）
...
function 生成图片HTML(数据URI) {
    return `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>edgetunnel</title><style>body{margin:0;padding:0;overflow:hidden;background:#000;display:flex;align-items:center;justify-content:center;height:100vh}img{width:100vw;height:100vh;object-fit:fill}</style></head><body><img src="${数据URI}" alt="homepage"></body></html>`;
}
// 图片URL正则检测（多种URL格式）
const 图片URL正则 = /\.(jpg|jpeg|png|gif|webp|bmp|svg|ico)(\?|$)|[?&]f=(jpg|jpeg|png|gif|webp|bmp|svg|ico)\b|\/image\/|\/img\/|\/photo\/|\/thumb\/|\/photo-|[?&](auto|fm|format)=(jpg|jpeg|png|gif|webp|bmp|svg|ico)\b/i;

// ★ 常用路径目录（随机路径用，全局复用避免重复分配）
...
const 登录限流计数 = new Map();  // 登录失败内存计数 { count, time }，1h 自动过期
const 订阅限流计数 = new Map();  // 订阅速率内存计数 { count, 小时桶 }，按小时桶对齐 KV（Fix1：KV 每 IP 每小时最多 1 写）
...
async function 加载永久黑名单(env) {
    if (!env.KV) return new Set();
    const now = Date.now();
    if (永久黑名单缓存 && (now - 永久黑名单缓存时间) < 永久黑名单缓存TTL) return 永久黑名单缓存;
    try {
        const raw = await withTimeout(env.KV.get('永久黑名单'), 3000);
...
async function 检查永久黑名单(客户端IP, env) {
    if (!客户端IP) return false;
    const 黑名单 = await 加载永久黑名单(env);
    return 黑名单.has(客户端IP);
}

async function 记录违规并检查拉黑(客户端IP, env, ctx) {
    if (!客户端IP || !env.KV) return;
    try {
        const key = `违规计数:${客户端IP}`;
        const raw = await withTimeout(env.KV.get(key), 3000);
        const 当前计数 = (raw ? Number(raw) : 0) + 1;
        const 黑名单 = await 加载永久黑名单(env);
        const 已永久封禁 = 黑名单.has(客户端IP);
        if (已永久封禁) {
            // 已永久黑名单 IP：计数持久保存，不重置（无 expirationTtl）
            ctx.waitUntil(env.KV.put(key, String(当前计数)));
        } else {
            // 非永久黑名单 IP：计数每周重置（7 天过期自动归零）
            ctx.waitUntil(env.KV.put(key, String(当前计数), { expirationTtl: 86400 * 7 }));
        }
        if (当前计数 >= 永久黑名单阈值 && !已永久封禁) {
            黑名单.add(客户端IP);
            永久黑名单缓存时间 = Date.now();
            ctx.waitUntil(env.KV.put('永久黑名单', JSON.stringify([...黑名单])));
...
// DoH 查询函数级 TTL 缓存（对齐上游 77c2097 优化思路：减少重复请求、负缓存避免反复查询不存在的域名）
const DoH函数缓存 = {};
const DoH函数缓存最大条目 = 256;
...
// 注意：connectionThrottles 是模块级 Map，仅在当前 CF Worker Isolate 内有效。
// 多 Isolate 场景下节流效果有限。如需跨 Isolate 节流，请使用 KV 或 Durable Objects。
...
let 启用SOCKS5反代, 启用SOCKS5全局反代, 我的SOCKS5账号;
...
const HMAC_SIGN_KEY_PREFIX = 特征码字典[1].slice(0,3).toUpperCase() + 特征码字典[1].slice(3) + 'ssss' + '_hmac_v1_';
...
function secureCompare(a, b) {
    ...
    return timingSafeEqualBytes(paddedA, paddedB);
}

// 恒定时间字节比较（防时序攻击；纯 JS 实现，兼容 Node.js WebCrypto 环境）
...
export default {
    async fetch(request, env, ctx) {
        ...
        const 访问路径 = url.pathname.slice(1).toLowerCase();
        if (env[特征码字典[0]]) {
            const proxyIPs = await 整理成数组(env[特征码字典[0]]);
        ...
        if (管理员密码 && upgradeHeader === 'websocket') {// WebSocket代理
            ...
            if (env.KV && typeof env.KV.get === 'function') {
                ...
                if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
                    const params = new URLSearchParams(url.search);
                    // [隐私增强] 可选加入时间随机盐（SUB_TOKEN_SALTED=true 时每小时变化）
                    const subTokenSalt = ['1', 'true'].includes(env.SUB_TOKEN_SALTED)
                        ? 加密秘钥 + Math.floor(Date.now() / 3600000).toString()
                        : '';
                    params.set('token', await MD5MD5(host + userID + subTokenSalt));
                    return new Response('重定向中...', { status: 302, headers: { 'Location': `/sub?${params.toString()}` } });
                } else if (访问路径 === 'login') {//处理登录页面和登录请求
                    const cookies = request.headers.get('Cookie') || '';
                    const authCookie = cookies.split(';').find(c => c.trim().startsWith('auth='))?.split('=')[1];
                    ...
                    if (request.method === 'POST') {
                        ...
                        const kv锁 = env.KV ? Number(await env.KV.get(`login_fail:${访问IP}`) || 0) : 0;
                        const 内存记录 = 登录限流计数.get(访问IP);
                        const 内存计数 = (内存记录 && Date.now() - 内存记录.time < 900000) ? 内存记录.count : 0;
                        if (kv锁 >= 5 || 内存计数 >= 5) return sanitizedErrorResponse('登录尝试次数过多，请15分钟后再试', 429);
                        ...
                        if (输入密码 && secureCompare(输入密码, typeof 管理员密码 === 'string' ? 管理员密码.replace(/[\r\n]/g, '') : 管理员密码)) {
                            ...
                            // Cookie: Secure; SameSite=Lax; Max-Age=3600(1小时)
                            const 响应 = new Response(JSON.stringify({ success: true }), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                            响应.headers.set('Set-Cookie', `auth=${authValue}; Path=/; Max-Age=3600; HttpOnly; Secure; SameSite=Lax`);
                            响应.headers.append('Set-Cookie', `auth_sig=${authSig}; Path=/; Max-Age=3600; HttpOnly; Secure; SameSite=Lax`);
                            return 响应;
                        } else {
                            // 失败：递增内存计数，仅跨阈值（>=5）时写 KV 持久锁
                            const 新次数 = 当前计数 + 1;
                            登录限流计数.set(访问IP, { count: 新次数, time: Date.now() });
                            if (新次数 >= 5 && env.KV) {
                                ctx.waitUntil(env.KV.put(`login_fail:${访问IP}`, String(新次数), { expirationTtl: 900 }));
                            }
                        }
                    }
                    ...
                    const authSig = cookies.split(';').find(c => c.trim().startsWith('auth_sig='))?.split('=')[1];
                    const expectedAuthValue = await MD5MD5(UA + 加密秘钥 + 管理员密码);
                    // 验证 cookie，未认证则跌落到伪装代理（不 return）；仅 admin/config.json 保持 401 供登录页鉴权探测
                    ...
                    if (访问路径 === 'admin/log.json') {// 读取日志内容
                        ...
                        if (url.searchParams.get('url')) {
                            ...
                            try {
                                new URL(待验证优选URL);
                                const 请求优选API内容 = await 请求优选API([待验证优选URL], url.searchParams.get('port') || '443');
                                const 优选API结果 = 请求优选API内容[0];
                                let 优选API的IP = 优选API结果 instanceof Map
                                    ? [...优选API结果.values()].flatMap(v => v.IPs || [])
                                    : (Array.isArray(优选API结果) ? 优选API结果 : []);
                                if (优选API的IP.length === 0) {
                                    优选API的IP = 优选API结果 instanceof Map
                                        ? [...优选API结果.values()].flatMap(v => v.Nodes || [])
                                        : (请求优选API内容[1] || []);
                                }
                                优选API的IP = 优选API的IP.map(item => item.replace(/#(.+)$/, (_, remark) => '#' + decodeURIComponent(remark)));
                                return new Response(JSON.stringify({ success: true, data: 优选API的IP }, null, 2), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                    ...
                    if (访问路径 === 'admin/init') {// 重置配置为默认值
                        ...
                        if (访问路径 === 'admin/config.json') { // 保存config.json配置
                            try {
                                ...
                                // [隐私增强] 加密存储配置内容（ENCRYPT_KEY 未设置时透传）
                                // ★ 变更检测（Fix4-2）：比对 KV 当前实际值（AES-CBC 随机 IV 密文必不同，只能比明文）；跨 isolate 可靠，KV 已是该值才跳过
                                const 待保存明文 = JSON.stringify(newConfig, null, 2);
                                let 当前KV明文 = null;
                                try {
                                    const 原始KV值 = await withTimeout(env.KV.get('config.json'), 3000);
                                    if (原始KV值) 当前KV明文 = await decryptKV(原始KV值, env);
                                } catch (e) { /* 读失败 → 当前KV明文=null → 走正常写入 */ }
                                if (当前KV明文 === 待保存明文) {
                                    // 未变更：跳过 KV 写 + 版本号，仅清缓存返回成功
                                ...
                                // 保存到 KV（带超时保护）
                                await withTimeout(env.KV.put('config.json', await encryptKV(待保存明文, env)), 3000);
                                // 更新版本号以通知全球节点清除 Cache API 缓存
                                ctx.waitUntil(env.KV.put('config_version', Date.now().toString()));
                                // 清除当前实例缓存
                                configJSONCache = null;
                            ...
                            try {
                                ...
                                // ★ 变更检测（Fix4-2）：比对 KV 当前实际值；CF_JSON 为重组对象，须在写前构造完再比较；跨 isolate 可靠
                                const 待保存明文 = JSON.stringify(CF_JSON, null, 2);
                                let 当前KV明文 = null;
                                try {
                                    const 原始KV值 = await withTimeout(env.KV.get('cf.json'), 3000);
                                    if (原始KV值) 当前KV明文 = await decryptKV(原始KV值, env);
                                } catch (e) { /* 读失败 → 当前KV明文=null → 走正常写入 */ }
                                if (当前KV明文 === 待保存明文) {
                                    configJSONCache = null;
                                ...
                                // 保存到 KV（带超时保护）
                                await withTimeout(env.KV.put('cf.json', await encryptKV(待保存明文, env)), 3000);
                                // 更新版本号
                                ctx.waitUntil(env.KV.put('config_version', Date.now().toString()));
                                // 清除当前实例缓存
                                configJSONCache = null;
                            ...
                            try {
                                ...
                                let TG_JSON;
                                if (newConfig.init && newConfig.init === true) {
                                    TG_JSON = { BotToken: null, ChatID: null };
                                } else {
                                    if (!newConfig.BotToken || !newConfig.ChatID) return new Response(JSON.stringify({ error: '配置不完整' }), { status: 400, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                                    TG_JSON = newConfig;
                                }
                                // ★ 变更检测（Fix4-2）：比对 KV 当前实际值；init 分支与普通分支各自明文；跨 isolate 可靠
                                const 待保存明文 = JSON.stringify(TG_JSON, null, 2);
                                let 当前KV明文 = null;
                                try {
                                    const 原始KV值 = await withTimeout(env.KV.get('tg.json'), 3000);
                                    if (原始KV值) 当前KV明文 = await decryptKV(原始KV值, env);
                                } catch (e) { /* 读失败 → 当前KV明文=null → 走正常写入 */ }
                                if (当前KV明文 === 待保存明文) {
                                    configJSONCache = null;
                                ...
                                // 保存到 KV（带超时保护）
                                await withTimeout(env.KV.put('tg.json', await encryptKV(待保存明文, env)), 3000);
                                // 更新版本号以通知全球节点清除 Cache API 缓存
                                ctx.waitUntil(env.KV.put('config_version', Date.now().toString()));
                                // 清除当前实例缓存
                                configJSONCache = null;
                            ...
                            try {
                                const customIPs = await request.text();
                                // ★ 变更检测（Fix4-2）：纯文本直接比对 KV 当前实际值；跨 isolate 可靠，KV 已是该值才跳过
                                let 当前ADD明文 = null;
                                try {
                                    const 原始KV值 = await withTimeout(env.KV.get('ADD.txt'), 3000);
                                    if (原始KV值) 当前ADD明文 = 原始KV值;
                                } catch (e) { /* 读失败 → 当前ADD明文=null → 走正常写入 */ }
                                if (当前ADD明文 === customIPs) {
                                    return new Response(JSON.stringify({ success: true, message: '自定义IP已保存' }), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                                }
                        ...
                        // [隐私增强] 仅返回诊断必需字段，移除 country/city/asn 等
                        const cfSafe = request.cf ? {
                            colo: request.cf.colo,
                            httpProtocol: request.cf.httpProtocol,
                            tlsVersion: request.cf.tlsVersion,
                        } : {};
                        return new Response(JSON.stringify(cfSafe, null, 2), { status: 200, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                    }

                    ctx.waitUntil(请求日志记录(env, request, 访问IP, 'Admin_Login', config_JSON));
                    const 管理页响应 = await fetchWithTimeout(Pages静态页面 + '/admin' + url.search, {}, 10000);
                    const 管理页HTML = await 管理页响应.text();
                    const 管理页背景CSS = `<style>
html, body { background: transparent !important; }
#background-iframe { position: fixed; top: 0; left: 0; width: 100%; height: 100%; border: 0; z-index: -3 !important; }
#blur-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: linear-gradient(135deg, rgba(102,126,234,0.1) 0, rgba(118,75,162,0.15) 100%); -webkit-backdrop-filter: blur(12px); backdrop-filter: blur(12px); z-index: -2 !important; animation: blurOverlayAppear 1.5s ease-out forwards .3s; pointer-events: none; opacity: 0; }
@keyframes blurOverlayAppear { 0% { opacity: 0; -webkit-backdrop-filter: blur(0px); backdrop-filter: blur(0px); } 100% { opacity: 1; -webkit-backdrop-filter: blur(12px); backdrop-filter: blur(12px); } }
</style>`;
                    const 管理页背景HTML = `<iframe id="background-iframe" src="/" title="背景内容"></iframe><div id="blur-overlay"></div>`;
                    const 管理页body插入位 = 管理页HTML.lastIndexOf('</body>');
                    const 管理页注入前置 = 管理页HTML.slice(0, 管理页body插入位).replace('</head>', 管理页背景CSS + '<script data-cfasync="false">window.__cfRLUnblockHandlers = true;</script></head>').replace('<body>', '<body style="background: transparent !important">');
                    const 管理页注入后置 = 管理页HTML.slice(管理页body插入位);
                    return new Response(管理页注入前置 + 管理页背景HTML + 管理页注入后置, { headers: { 'Content-Type': 'text/html; charset=utf-8' } });
                    } else if (访问路径 === 'admin/config.json') {
                        // 登录页鉴权探测端点：保持 401，避免登录页误判已登录而自动跳转
                        return new Response(JSON.stringify({ error: '未认证' }), { status: 401, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
                    }
                    // 其它未认证 admin 路径：不 return，跌落到伪装页（200）
                } else if (访问路径 === 'logout' || uuidRegex.test(访问路径)) {//清除cookie并跳转到登录页面
                    const 响应 = new Response('重定向中...', { status: 302, headers: { 'Location': '/login' } });
                    ...
                    const 订阅转换后端请求订阅 = 请求TOKEN === 今日订阅转换后端专属TOKEN || 请求TOKEN === 昨日订阅转换后端专属TOKEN;
                    if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
                        // 订阅速率限制：单IP 10次/小时（按小时桶 KV 键，仅桶首次创建写 KV，避免每请求写）
                        const 当前小时桶 = Math.floor(Date.now() / 3600000);
                        const 小时桶键 = `sub_rate:${访问IP}:${当前小时桶}`;
                        const kv桶计数 = env.KV ? Number(await env.KV.get(小时桶键) || 0) : 0;
                        const 内存记录 = 订阅限流计数.get(访问IP);
                        const 内存计数 = (内存记录 && 内存记录.小时桶 === 当前小时桶) ? 内存记录.count : 0;
                        if (kv桶计数 >= 10 || 内存计数 >= 10) {
                            return sanitizedErrorResponse('订阅请求过于频繁，请稍后重试', 429);
                        }
                        const 当前计数 = Math.max(kv桶计数, 内存计数);
                        const 新计数 = 当前计数 + 1;
                        订阅限流计数.set(访问IP, { count: 新计数, 小时桶: 当前小时桶 });
                        // 仅当该小时桶在 KV 不存在时写入（每 IP 每小时最多 1 次 KV put；TTL 2h 覆盖桶边界）
                        if (kv桶计数 === 0 && env.KV) {
                            ctx.waitUntil(env.KV.put(小时桶键, String(新计数), { expirationTtl: 7200 }));
                        }
                        ...
                        if (['mixed', 'base64', 'v2ray'].includes(订阅类型)) {
                            const TLS分片参数 = 获取TLS分片参数(config_JSON);
                            const isLoonOrSurge = ua.includes('loon') || ua.includes('surge');
                            ...
                            let 完整优选IP = [], 其他节点LINK = '', 反代IP池 = [];
                                ...
                                const IP转换节点 = async (输入元素) => {
                                    ...
                                    const ECH_SNI = 获取ECH_SNI(config_JSON, host);
                                ...
                                if (链式代理匹配) {
                                    try {
                                     ...
                                     const 匹配到的反代IP = 反代IP池.find(p => p.includes(节点地址));
                                     if (匹配到的反代IP) 完整节点路径 = (`${config_JSON.PATH}/${特征码字典[0].toLowerCase()}=${匹配到的反代IP}`).replace(/\/\//g, '/') + (config_JSON.启用0RTT ? '?ed=2560' : '');
                                 }
                                if (isLoonOrSurge) 完整节点路径 = 完整节点路径.replace(/,/g, '%2C');
                                ...
                                if (协议类型 === 'ss') {
                                    ...
                                    const 传输路径参数值 = 获取传输路径参数值(config_JSON, 完整节点路径, 作为优选订阅生成器);
                                    return `${协议类型}://${uuidToUse}@${节点地址}:${节点端口}?security=tls&type=${传输协议 + ECHLINK参数}&${域名字段名}=example.com&fp=${config_JSON.Fingerprint}&sni=example.com&${路径字段名}=${encodeURIComponent(传输路径参数值) + TLS分片参数}&encryption=none&alpn=${encodeURIComponent(config_JSON.ALPN)}${config_JSON.跳过证书验证 ? '&insecure=1&allowInsecure=1' : ''}#${encodeURIComponent(节点备注)}`;
                                }
                            };
                            ...
                            const 提取IP键 = (ip值) => {
                                ...
                                // 裸 IPv6（纯十六进制+冒号，无括号）→ 补括号 + 缺省端口（与 cfnew 提取节点去重键 语义一致）
                                if (/^[0-9a-fA-F:]+$/.test(无备注)) return '[' + 无备注 + ']:443';
                                return 无备注;
                            };
                            const 是IP已处理 = (ip值) => {
                                const 键 = 提取IP键(ip值);
                            ...
                            if (!url.searchParams.has('sub') && config_JSON.优选订阅生成.local) { // 本地生成订阅
                                ...
                                for (const 元素 of 完整优选列表) {
                                    ...
                                    if (元素.toLowerCase().startsWith('sub://')) {
                                        ...
                                        const subMatch = 元素.match(/sub\s*=\s*([^\s&#]+)/i);
                                        if (subMatch && subMatch[1].trim().includes('.')) {
const 优选IP作为反代IP = 元素.toLowerCase().includes(特征码字典[0].toLowerCase() + '=true');
const apiValue = 'sub://' + subMatch[1].trim() + (优选IP作为反代IP ? '?' + 特征码字典[0].toLowerCase() + '=true' : "") + (元素.includes('#') ? ('#' + 元素.split('#')[1]) : '');
                                            优选API.push(apiValue);
                            ...
                            // 当加密启用且 envAF 已设置时，自动注入 af 参数确保转换器回源请求不加密
                            const 加密启用 = !!(管理员密码 || 加密秘钥);
                            const needsAF = 加密启用 && (env.AF || env.af) && !queryAF ? `&af=${encodeURIComponent(env.AF || env.af)}` : '';
                            const cnIspCodeParam = `&cnIspCode=${识别运营商(request)}`;
                            const 订阅转换URL = `${config_JSON.订阅转换配置.SUBAPI}/sub?target=${订阅类型}&url=${encodeURIComponent(url.protocol + '//' + url.host + '/sub?target=mixed&token=' + 今日订阅转换后端专属TOKEN + cnIspCodeParam + (url.searchParams.has('sub') && url.searchParams.get('sub') != '' ? `&sub=${url.searchParams.get('sub')}` : '') + (queryAF ? `&af=${encodeURIComponent(queryAF)}` : '') + needsAF)}&config=${encodeURIComponent(config_JSON.订阅转换配置.SUBCONFIG)}&emoji=${config_JSON.订阅转换配置.SUBEMOJI}&list=${config_JSON.订阅转换配置.SUBLIST}&scv=${config_JSON.跳过证书验证}&xudp=${config_JSON.订阅转换配置.XUDP}&udp=${config_JSON.订阅转换配置.UDP}&tls13=${config_JSON.订阅转换配置.TLS13}&append_type=${config_JSON.订阅转换配置.APPEND_TYPE}&sort=${config_JSON.订阅转换配置.SORT}&expand=${config_JSON.订阅转换配置.EXPAND}`;
                            try {
                                const response = await fetch(订阅转换URL, { headers: { 'User-Agent': 'Subconverter for ' + 订阅类型 + ' edge' + 'tunnel (https://github.com/' + 特征码字典[1] + '/edge' + 'tunnel)' } });
                                if (response.ok) {
                                    // 逐行读取后端转换响应，每行 yield 避免 CPU 超时
                                    ...
                                    while (true) {
                                        ...
                                        for (const 行 of 行列表) {
                                            订阅内容 += 行 + '\n';
                                            行计数++;
                                            if (行计数 % YIELD_INTERVAL === 0) await yieldToEventLoop();
                                        }
                                    }
                                    if (缓冲区) 订阅内容 += 缓冲区;
        ...
        // ★ 速率限制 + 永久黑名单：非管理路径的请求段必须在常用路径目录中，否则限流（每 IP 每小时最多 6 次）
        if (管理员密码 && request.method === 'GET' && !访问路径.startsWith('admin/') && 访问路径 !== 'login' && 访问路径 !== 'version') {
            const 路径段 = 访问路径.split('/').filter(Boolean);
            if (路径段.length > 0 && !路径段.every(段 => 常用路径目录.includes(段))) {
                const 客户端IP = request.headers.get('CF-Connecting-IP') || '';
                const hsts头 = ['1', 'true'].includes(env.HSTS_ENABLE) ? {'Strict-Transport-Security': 'max-age=31536000; includeSubDomains; preload'} : {};
                if (await 检查永久黑名单(客户端IP, env)) {
                    console.warn(`[永久黑名单] IP ${客户端IP} 命中永久黑名单，直接拦截`);
                    return new Response(await html1101(url.host, 客户端IP), { status: 200, headers: { 'Content-Type': 'text/html; charset=UTF-8', ...hsts头 } });
                }
                if (!检查速率限制(客户端IP)) {
                    console.warn(`[速率限制] IP ${客户端IP} 超过非管理员路径请求限制`);
                    ctx.waitUntil(记录违规并检查拉黑(客户端IP, env, ctx));
                    return new Response(await html1101(url.host, 客户端IP), { status: 200, headers: { 'Content-Type': 'text/html; charset=UTF-8', ...hsts头 } });
                }
                记录速率限制请求(客户端IP);
            }
        ...
        if (伪装页URL && 伪装页URL !== 'nginx' && 伪装页URL !== '1101') {
            伪装页URL = 伪装页URL.trim().replace(/\/$/, '');
            // ★ 多地址支持：竖线分隔，随机抽取一个
            const 全部地址列表 = 伪装页URL.split('|').map(u => u.trim()).filter(u => u);
            if (全部地址列表.length > 0) 伪装页URL = 全部地址列表[Math.floor(Math.random() * 全部地址列表.length)];
            if (!伪装页URL.match(/^https?:\/\//i)) 伪装页URL = 'https://' + 伪装页URL;
            if (伪装页URL.toLowerCase().startsWith('http://')) 伪装页URL = 'https://' + 伪装页URL.substring(7);
        }
        // ★ 图片URL检测（需在 host-only 归一化之前，保持完整路径用于抓取）
        if (伪装页URL && 伪装页URL !== 'nginx' && 伪装页URL !== '1101' && 图片URL正则.test(伪装页URL)) {
            try {
                const 缓存命中 = 伪装图片缓存.get(伪装页URL);
                if (缓存命中) {
                ...
                const 图片源 = new URL(伪装页URL);
                const 图片反代响应 = await fetchWithTimeout(伪装页URL, {
                    headers: {
                ...
                // 正则假阳性保护：服务器返回 HTML 说明不是图片，降级到普通反代
                ...
                if (图片长度 > 0 && 图片长度 < 2 * 1024 * 1024) {
                    ...
                    if (图片字节.byteLength < 2 * 1024 * 1024) {
                        ...
                        伪装图片缓存.set(伪装页URL, { body: 图片字节, contentType: 内容类型, time: Date.now() });
                        const 数据URI = 字节转HTML图片(图片字节, 内容类型);
                        return new Response(生成图片HTML(数据URI), { status: 200, headers: { 'Content-Type': 'text/html; charset=utf-8', ...hsts头 } });
                    }
                    // content-length 虚标：改为纯流式
        ...
        try {
            ...
            // Phase 3b: 清除 CF 相关入站头，避免透传给目标服务器（先克隆再删除，避免修改不可变 request.headers）
            ...
            try {
                清理伪装页缓存();
                const 反代响应 = await fetchWithTimeout(反代URL.origin + url.pathname + url.search, fetchOptions, 39000);
                const 内容类型 = 反代响应.headers.get('content-type') || '';
...
async function 处理XHTTP请求(request, yourUUID, ctx2) {
    ...
    if (首包.isUDP) return 处理叉HTTPUDP请求(首包, reader, ctx2, responseHeaders, 递减连接数);
    ...
    try {
        ...
        socket = await forwardataTCP(首包.hostname, 首包.port, rawData, 占位WS, 首包.respHeader, remoteConnWrapper, yourUUID, ctx2, 首包.协议 === 'trojan', ctx2.木马反代地址, 首包.原始数据, true);
    } catch (err) {
            ...
            try {
                while (true) {
                    ...
                    if (value?.byteLength) {
                        ...
                        if (有效Value?.byteLength) await 上行合包器.写入(有效Value);
                    }
                }
            } finally {
...
function 处理叉HTTPUDP请求(首包, reader, ctx2, responseHeaders, 递减连接数) {
    const log = (...args) => ctx2.log(...args);
        ...
        async start(controller) {
            ...
            try {
                if (首包.协议 === 'trojan') {
                    ...
                    if (木马UDP上下文.反代地址) await 转发木马UDP数据(首包.原始数据, 叉桥, 木马UDP上下文, ctx2);
                }
                if (!(首包.协议 === 'trojan' && 木马UDP上下文.反代地址) && 首包.rawData?.byteLength) {
                    if (首包.协议 === 'trojan') await 转发木马UDP数据(首包.rawData, 叉桥, 木马UDP上下文, ctx2);
                    else await forwardataudp(首包.rawData, 叉桥, udpRespHeader, null, ctx2);
                    udpRespHeader = null;
                }
                while (true) {
                    const { done, value } = await reader.read();
                    ...
                    if (首包.协议 === 'trojan') await 转发木马UDP数据(value, 叉桥, 木马UDP上下文, ctx2);
                    else await forwardataudp(value, 叉桥, udpRespHeader, null, ctx2);
                    udpRespHeader = null;
                }
            } catch (err) {
                转发失败 = true;
                        ...
                        if (remoteConnWrapper.socket || remoteConnWrapper.connectingPromise) {
                            ...
                            if (判断是否是木马) {
                                ...
                                if (isUDP) {
                                    ...
                                    木马UDP上下文.目标端口 = port;
                                    if (木马UDP上下文.反代地址) await 转发木马UDP数据(有效首包, grpcBridge, 木马UDP上下文, ctx2);
                                    else if (有效数据长度(rawClientData) > 0) await 转发木马UDP数据(rawClientData, grpcBridge, 木马UDP上下文, ctx2);
                                } else {
                                    ...
                                    await forwardataTCP(hostname, port, rawData, grpcBridge, null, remoteConnWrapper, yourUUID, ctx2, 判断是否是木马, ctx2.木马反代地址, 有效首包);
                                }
                            } else {
                                判断是否是木马 = false;
                                ...
                                else await forwardataTCP(hostname, port, rawData, grpcBridge, null, remoteConnWrapper, yourUUID, ctx2, 判断是否是木马, ctx2.木马反代地址, rawData);
                            }
                        }
                    }
                    刷新发送队列();
...
async function 处理WS请求(request, yourUUID, url, ctx2) {
	...
	if (!SS模式禁用EarlyData && earlyData) {
		try {
			...
			if (bytes?.byteLength) {
				...
				// 不中断握手：无效 early data 仅记录日志，继续以空 early data 走 101 升级
			}
		} catch (error) {
	...
	// ★ 修复: 移除 allowHalfOpen — 客户端先行关闭时服务端随之关闭，WebSocketPair 完全终止，
	// 避免 workerd 运行时判定请求挂起 (outcome: exception + "code had hung")。
	// 下行发送均已校验 readyState === OPEN，半开态无依赖，行为不变。
	serverSock.accept();
	...
	const 处理SS数据 = async (chunk) => {
		...
		for (const 明文块 of 明文块数组) {
			...
			await forwardataTCP(hostname, port, rawData, 上下文.回包Socket, null, remoteConnWrapper, yourUUID, ctx2, 判断是否是木马, ctx2.木马反代地址, rawData);
		}
	};
	...
	const 处理WS入站数据 = async (chunk) => {
		...
		if (判断协议类型 === null) {
			...
			判断是否是木马 = 判断协议类型 === '木马';
			log(`[WS转发] 协议类型: ${判断协议类型} | 来自: ${sanitizeHostname(url.host)} | UA: ${request.headers.get('user-agent') || '未知'}`);
		}
		...
		if (判断协议类型 === '木马') {
			...
			if (isUDP) {
				...
				木马UDP上下文.目标主机 = hostname;
				木马UDP上下文.目标端口 = port;
				if (木马UDP上下文.反代地址) return 转发木马UDP数据(有效块, serverSock, 木马UDP上下文, ctx2);
			...
			await forwardataTCP(hostname, port, rawData, serverSock, null, remoteConnWrapper, yourUUID, ctx2, 判断是否是木马, ctx2.木马反代地址, 有效块);
		} else {
			判断是否是木马 = false;
			当前块字节 = 当前块字节 || 有效块;
			const bytes = 当前块字节;
			...
			await forwardataTCP(hostname, port, rawData, serverSock, respHeader, remoteConnWrapper, yourUUID, ctx2, 判断是否是木马, ctx2.木马反代地址, rawData);
		}
	};
	...
	const 收尾WS显式传输 = () => {
		...
		追加WS显式传输任务(async () => {
			if (WS显式传输失败) return;
			// ★ 修复: 限定排空等待时间，避免上游连接悬挂时收尾链任务永久挂起
			await Promise.race([
				上行写入队列.等待空(),
	...
	serverSock.addEventListener('close', () => {
		closeSocketQuietly(serverSock);
		// ★ 修复: 同步失效远端连接 — 立即关闭上游 socket 并抬升世代，
		// 使在途拨号/写入即刻失败(安装当前连接检查世代不匹配)，加速请求上下文终结。
		// 幂等: socket 已置空，重复调用仅再抬升一次世代，无副作用。
		失效远端连接();
		收尾WS显式传输();
	});
...
async function 转发木马UDP数据(chunk, webSocket, 上下文, ctx2) {
	...
	// Trojan回退: 如果配置了回退地址，走TCP转发（首次调用会自动创建socket）
	if (上下文?.反代地址) return 转发木马UDP反代数据(当前块, webSocket, 上下文, ctx2.request);
...
async function SSHKDFSHA1(ikm, salt, info, outputLen) {
	...
	while (written < outputLen) {
		const input = 拼接字节数据(previous, info, new Uint8Array([counter]));
...
function 创建SNI混合器(config = {}, env = null) {
    ...
    let 真实SNI比例范围 = config.真实SNI比例范围 ??
        (config.真实SNI比例 != null ? [config.真实SNI比例, config.真实SNI比例] : [0.71, 0.81]);
    // 防御：归一化点外若传入标量，包装为数组
    if (!Array.isArray(真实SNI比例范围) && 真实SNI比例范围 != null) {
        真实SNI比例范围 = [真实SNI比例范围, 真实SNI比例范围];
    }
    ...
    let 需Flush = false;
    let 上次KVFlush时间 = 0;
    let 上次同步标记刷新时间 = 0;  // 同步标记 TTL 刷新节流（Fix3）：12h 才刷一次，避免每次 flush 同值重写
    const KV_FLUSH最小间隔 = 180000; // 180秒（3分钟）最小间隔 — KV免费写入限额1000/天，每次flush约2次put，480次*2=960写/天 < 1000安全线
    ...
    function 选择SNI(真实SNI) {
        ...
        // ★ 均匀随机 [min, max]，无周期性指纹，每请求独立采样
        const currentRatio = min + Math.random() * (max - min);
        ...
        // 真实SNI 几乎不在白名单中（白名单为国内 CDN 域名），跳过 filter 避免 O(n) 分配
        if (!白名单.includes(真实SNI) && 白名单.length > 0) {
            混合计数.掩护++;
            return 白名单[Math.floor(Math.random() * 白名单.length)];
        }
    ...
    async function flushToKV(env) {
        ...
        // 惰性写入：至少间隔 KV_FLUSH最小间隔 才实际写入 KV，避免高频请求打爆写入限额
        const 当前时间 = Date.now();
        if (当前时间 - 上次KVFlush时间 < KV_FLUSH最小间隔) return null;
        上次KVFlush时间 = 当前时间;
        try {
            ...
            if (state !== '1') {
                // ✅ 快照当前增量，避免并发竞争丢失
                const snapshot = { ...混合计数 };
                // ✅ 修复 &reset 后"无数据": 有数据则写入 KV，避免静默丢弃
                if (snapshot.总计 > 0) {
                    await env.KV.put(kvKey, JSON.stringify(snapshot));
                } else {
                ...
                混合计数.掩护 -= snapshot.掩护;
                混合计数.总计 -= snapshot.总计;
                需Flush = false;  // 并发插入随下次选择SNI自然触发flush，避免连续KV写
                return snapshot;  // ✅ 返回数据，供获取聚合统计直接展示
            }
            // 刷新同步标记 TTL（86400s = 24h，关同步后标记过期 → 下次开同步自动重置）；同值去重（Fix3）：12h 才刷一次
            if (Date.now() - 上次同步标记刷新时间 > 43200000) {
                await env.KV.put('sni_mixing_sync_active', '1', { expirationTtl: 86400 });
                上次同步标记刷新时间 = Date.now();
            }
    ...
    // ★ 先清本地（同步）再清 KV（异步），避免 flushToKV 在 await 间隙读到 snapshot=非0 后写出负数
...
function 确保SNI混合器已初始化(env) {
    if (全局SNI混合器) return;
    const config = { 启用: false, 真实SNI比例范围: [0.71, 0.81], 白名单: 默认SNI白名单 };
    if (env.SNIMIXING) {
        try {
            const envConfig = JSON.parse(env.SNIMIXING);
            // 兼容旧字段名: 真实SNI比例 → 真实SNI比例范围（标量 → 数组）
            if (envConfig.真实SNI比例 != null && envConfig.真实SNI比例范围 == null) {
                envConfig.真实SNI比例范围 = Array.isArray(envConfig.真实SNI比例)
                    ? envConfig.真实SNI比例
                    : [envConfig.真实SNI比例, envConfig.真实SNI比例];
            }
...
class TlsClient {
    ...
    setStage(stage) {
        if (!(stage in this.yieldThresholds)) {
            console.warn(`[TlsClient] 未知阶段: ${stage}，使用 transmission 阈值`);
            this.currentStage = 'transmission';
        ...
        async readHandshakeUntil(reader, predicate, closedError) {
            ...
            return this.readRecordsUntil(reader, async (record) => {
                ...
                if (this.isTls13 && record.type === CONTENT_TYPE_APPLICATION_DATA) {
                    // TLS 1.3：使用握手流量密钥解密加密的握手消息
                    // decryptTls13Handshake 返回裸 Uint8Array（含内层 content_type 字节，仅剥尾部零填充）
	...
	async receiveServerHello(reader) {
        ...
        async handshakeTls13(reader, writer, serverHello) {
            ...
            // ==================== RFC 8446 §7.1 密钥派生（第一阶段：握手流量密钥） ====================
            // 2.1 early_secret = HKDF-Extract(salt=0, IKM=0)  // 无 PSK 场景
            const zeroHash = new Uint8Array(hashLen); // 动态哈希输出长度
            const earlySecret = await hkdfExtract(tls13Hash, zeroHash, new Uint8Array(0));

            // 2.2 derived_secret = Derive-Secret(early_secret, "derived", "")
            const derivedSecret = await hkdfExpandLabel(tls13Hash, earlySecret, 'derived', await TLS摘要(tls13Hash, TLS_EMPTY_BYTES), hashLen);

            // 2.3 handshake_secret = HKDF-Extract(salt=derived_secret, IKM=shared_secret)
            const handshakeSecret = await hkdfExtract(tls13Hash, derivedSecret, sharedSecret);
            this.handshakeSecret = handshakeSecret;

            // 2.4 派生 client/server handshake traffic secret（上下文 = ClientHello..ServerHello 的 transcript hash）
            const transcriptHash = await TLS摘要(tls13Hash, this.transcript());
            const clientHandshakeTrafficSecret = await hkdfExpandLabel(tls13Hash, handshakeSecret, 'c hs traffic', transcriptHash, hashLen);
            const serverHandshakeTrafficSecret = await hkdfExpandLabel(tls13Hash, handshakeSecret, 's hs traffic', transcriptHash, hashLen);

            // 2.5 从 traffic secret 派生握手流量密钥（key / iv）
            const clientHandshakeKey = await hkdfExpandLabel(tls13Hash, clientHandshakeTrafficSecret, 'key', new Uint8Array(0), keyLen);
            const serverHandshakeKey = await hkdfExpandLabel(tls13Hash, serverHandshakeTrafficSecret, 'key', new Uint8Array(0), keyLen);
            const clientHandshakeIv = await hkdfExpandLabel(tls13Hash, clientHandshakeTrafficSecret, 'iv', new Uint8Array(0), ivLen);
            const serverHandshakeIv = await hkdfExpandLabel(tls13Hash, serverHandshakeTrafficSecret, 'iv', new Uint8Array(0), ivLen);
            ...
            // 2.6 server_finished_key（用于校验服务器 Finished 的 verify_data）
            const serverFinishedKey = await hkdfExpandLabel(tls13Hash, serverHandshakeTrafficSecret, 'finished', new Uint8Array(0), hashLen);

            // 3. 读取并解密服务器握手消息（EncryptedExtensions + CertificateRequest? + Certificate + CertificateVerify + Finished）
            //    握手流量密钥此时已就绪，readHandshakeUntil 的 TLS 1.3 分支可用 decryptTls13Handshake 解密
            await this.readHandshakeUntil(reader, async (message) => {
                switch (message.type) {
                    ...
                    case HANDSHAKE_TYPE_FINISHED:
                        // RFC 8446 §4.4.4：校验服务器 Finished 的 verify_data（transcript 此时不含 server Finished）
                        const expectedServerVerifyData = await TLS_HMAC(tls13Hash, serverFinishedKey, await TLS摘要(tls13Hash, this.transcript()));
                        if (!constantTimeEqual(expectedServerVerifyData, message.body)) throw new Error('TLS 1.3 server Finished verify failed');
                        // 校验通过后再将 server Finished 计入 transcript（否则客户端 Finished 的 transcript hash 缺 server Finished）
                        this.recordHandshake(message.raw);
                        return true;
                    default:
                        this.recordHandshake(message.raw);
            ...
            // ==================== RFC 8446 §7.1 密钥派生（第二阶段：应用流量密钥） ====================
            // 4.1 master_secret = HKDF-Extract(salt=Derive-Secret(handshake_secret, "derived", ""), IKM=0)
            const masterDerivedSecret = await hkdfExpandLabel(tls13Hash, handshakeSecret, 'derived', await TLS摘要(tls13Hash, TLS_EMPTY_BYTES), hashLen);
            const masterSecret = await hkdfExtract(tls13Hash, masterDerivedSecret, new Uint8Array(0));
            this.masterSecret = masterSecret;

            // 4.2 派生 client/server application traffic secret（上下文 = ClientHello..server Finished 的 transcript hash）
            const applicationTranscriptHash = await TLS摘要(tls13Hash, this.transcript());
            const clientAppTrafficSecret = await hkdfExpandLabel(tls13Hash, masterSecret, 'c ap traffic', applicationTranscriptHash, hashLen);
            const serverAppTrafficSecret = await hkdfExpandLabel(tls13Hash, masterSecret, 's ap traffic', applicationTranscriptHash, hashLen);

            // 4.3 从 traffic secret 派生应用流量密钥（key / iv）
            const clientAppKey = await hkdfExpandLabel(tls13Hash, clientAppTrafficSecret, 'key', new Uint8Array(0), keyLen);
            const serverAppKey = await hkdfExpandLabel(tls13Hash, serverAppTrafficSecret, 'key', new Uint8Array(0), keyLen);
            const clientAppIv = await hkdfExpandLabel(tls13Hash, clientAppTrafficSecret, 'iv', new Uint8Array(0), ivLen);
            const serverAppIv = await hkdfExpandLabel(tls13Hash, serverAppTrafficSecret, 'iv', new Uint8Array(0), ivLen);
            ...
            // 4.5 TLS 1.3 客户端证书：发送空证书完成握手
            if (sawCertificateRequest) {
                const emptyCertificate = buildHandshakeMessage(HANDSHAKE_TYPE_CERTIFICATE, tlsBytes(0, 0, 0, 0));
                this.recordHandshake(emptyCertificate);
                await writer.write(buildTlsRecord(CONTENT_TYPE_APPLICATION_DATA, await this.encryptTls13Handshake(concatBytes(emptyCertificate, [CONTENT_TYPE_HANDSHAKE]))));
            }

            // 5. 发送客户端 Finished (RFC 8446 §4.4.4)
            // 5.1 计算 client_finished_key（从 client_handshake_traffic_secret 派生）
            const clientFinishedKey = await hkdfExpandLabel(tls13Hash, clientHandshakeTrafficSecret, 'finished', new Uint8Array(0), hashLen);
            // 5.2 计算 verify_data = HMAC(client_finished_key, Hash(transcript))（transcript 已含 server Finished）
            const verifyData = await TLS_HMAC(tls13Hash, clientFinishedKey, await TLS摘要(tls13Hash, this.transcript()));
            // 5.3 构建 Finished 握手消息
            const clientFinished = buildHandshakeMessage(HANDSHAKE_TYPE_FINISHED, verifyData);
            // 5.4 将客户端 Finished 计入 transcript（服务器将据此校验）
            this.recordHandshake(clientFinished);
            // 5.5 使用握手流量密钥加密并发送；encryptTls13Handshake 不附加内层 content_type，
            //     需手工 concatBytes 附加 [CONTENT_TYPE_HANDSHAKE]（RFC 8446 Finished 内层 content_type 为 handshake）
            const encryptedFinished = await this.encryptTls13Handshake(concatBytes(clientFinished, [CONTENT_TYPE_HANDSHAKE]));
            await writer.write(buildTlsRecord(CONTENT_TYPE_APPLICATION_DATA, encryptedFinished, TLS_VERSION_12));

            // 6. RFC 8446 §5.3：握手密钥阶段结束后、应用密钥阶段开始前重置序列号
            this.clientSeqNum = 0n;
            this.serverSeqNum = 0n;
	...
	async buildTls12Finished(secret, transcript) {
		...
		const finishedMessage = buildHandshakeMessage(HANDSHAKE_TYPE_FINISHED, verifyData);
		this.recordHandshake(finishedMessage);
		return buildTlsRecord(CONTENT_TYPE_HANDSHAKE, await this.encryptTls12(finishedMessage, CONTENT_TYPE_HANDSHAKE));
	}
...
async function 创建SS出站加密器(config, passwordText) {
	...
	return {
		async 加密(dataChunk) {
			...
			if (plaintextData.byteLength === 0) return 拼接字节数据(...outboundChunks);
			let offset = 0;
			while (offset < plaintextData.byteLength) {
...
async function forwardataTCP(host, portNum, rawData, ws, respHeader, remoteConnWrapper, yourUUID, ctx2, isTrojan = false, 木马反代地址 = null, 原始数据 = null, 仅建立连接 = false) {
	const { 反代IP, 启用反代兜底, 启用SOCKS5反代, 启用SOCKS5全局反代, parsedSocks5Address, SOCKS5白名单正则, 调试日志打印, TCP并发拨号数, 反代并发拨号数, 预加载竞速拨号 } = ctx2;
	const log = (...args) => ctx2.log(...args);
    ...
    async function connectProxyIP(address, port, data = null, 所有反代数组 = null, 启用反代失败兜底 = true) {
        if (所有反代数组 && 所有反代数组.length > 0) {
            ...
            for (let i = 0; i < 所有反代数组.length && 已尝试批次 < 最大重试深度; i += 实际并发数) {
                const 候选列表 = [];
...
async function forwardataudp(udpChunk, webSocket, respHeader, 响应封装器 = null, ctx2) {
    ...
    // DNS 缓存查询：用完整查询字节做缓存键，避免事务 ID 碰撞
    if (请求字节数 >= 2) {
        const cacheKey = Array.from(请求数据).join(',');
...
function 构造WS本地204响应(respHeader = null) {
	const WS本地204响应 = $encoder.encode(
		'HTTP/1.1 204 No Content\r\n' +
		'Content-Length: 0\r\n' +
...
async function turnConnect(proxy, targetHost, targetPort, TCP连接 = null) {
	...
	try {
		...
		if (
			...
			integrityKey = MD5纯JS(SS文本编码器.encode(`${proxy.username}:${realm}:${proxy.password}`));
			authAttributes = [
...
async function sstpConnect(proxy, targetHost, targetPort, TCP连接 = null) {
	...
	try {
		...
		const tlsServerName = isIPHostname(serverHost) ? '' : serverHost;
		let echConfig = null, echEnabled = false;
		if (config_JSON?.ECH出站 && !isIPHostname(serverHost) && typeof 获取ECHConfig === 'function') {
			try {
...
async function socks5Connect(targetHost, targetPort, initialData, TCP连接 = null, socks5Addr) {
    const { username, password, hostname, port } = socks5Addr;
...
async function httpsConnect(targetHost, targetPort, initialData, TCP连接 = null, socks5Addr, allowInsecure = config_JSON?.跳过证书验证 ?? false) {
	const { username, password, hostname, port } = socks5Addr;
...
async function httpConnect(targetHost, targetPort, initialData, TCP连接 = null, socks5Addr) {
    const { username, password, hostname, port } = socks5Addr;
    ...
    try {
        ...
        // CONNECT 响应头后可能夹带数据，回灌到可读流（避免首包丢失）
...
function 获取传输协议配置(config_JSON) {
	const 是gRPC = config_JSON.传输协议 === 'grpc';
	const { 头: 本机Padding头, 键: 本机Padding键 } = 获取叉HTTPPadding标识(config_JSON.UUID);
	const 叉混淆JSON = {
	...
	return {
		type: 是gRPC ? (config_JSON.gRPC模式 === 'multi' ? 'grpc&mode=multi' : 'grpc&mode=gun') : (config_JSON.传输协议 === 'xhttp' ? `xhttp&mode=stream-one&extra=${encodeURIComponent(JSON.stringify(叉混淆JSON))}` : 'ws'),
		路径字段名: 是gRPC ? 'serviceName' : 'path',
		域名字段名: 是gRPC ? 'authority' : 'host'
	};
}

function 获取TLS分片参数(config_JSON) {
	const 分片 = config_JSON.TLS分片 == 'Shadowrocket' ? `&fragment=${encodeURIComponent('1-36,70-700,100-200,tlshello')}` : config_JSON.TLS分片 == 'Happ' ? `&fragment=${encodeURIComponent('70-700,1-36,tlshello,100-200')}` : '';
	const 填充 = config_JSON.TLS填充 ? `&padding=${encodeURIComponent('0-128')}` : '';
	return 分片 + 填充;
}
...
function 获取传输路径参数值(config_JSON, 完整节点路径, 作为优选订阅生成器) {
	...
	if (config_JSON.传输协议 !== 'grpc') return 传输路径参数值;
	return 传输路径参数值.split('?')[0] || '/';
}
...
async function Clash订阅配置文件热补丁(Clash_原始订阅内容, uuid = null, ECH启用 = false, HOSTS = [], ECH_SNI = null, ECH_DNS, gRPCUserAgent = null) {
    ...
    const 添加Block格式gRPCUserAgent = (nodeLines, topLevelIndent) => {
        const 缩进 = ' '.repeat(topLevelIndent);
        let grpcOptsIndex = -1;
        ...
        if (grpcOptsIndex === -1) {
            ...
            if (insertIndex >= 0) nodeLines.splice(insertIndex + 1, 0, `${缩进}grpc-opts:`, `${缩进}  grpc-user-agent: ${gRPCUserAgentYAML}`);
            return nodeLines;
        }
        ...
        let 子缩进 = topLevelIndent + 2;
        let 已存在gRPCUserAgent = false;
        for (let idx = grpcOptsIndex + 1; idx < nodeLines.length; idx++) {
            const line = nodeLines[idx];
            const trimmed = line.trim();
            if (!trimmed) continue;
            const indent = line.search(/\S/);
            if (indent <= topLevelIndent) { blockEndIndex = idx; break; }
            if (indent > topLevelIndent && 子缩进 === topLevelIndent + 2) 子缩进 = indent;
            if (/^grpc-user-agent\s*:/.test(trimmed)) { 已存在gRPCUserAgent = true; break; }
        }
        if (!已存在gRPCUserAgent) nodeLines.splice(blockEndIndex, 0, `${' '.repeat(子缩进)}grpc-user-agent: ${gRPCUserAgentYAML}`);
        return nodeLines;
    };
...
<<<<<<< local_worker.js
    ...
    // 如果没有需要处理gRPC 且 ECH 未启用，直接返回
    if (!需要处理gRPC && !ECH启用) return clash_yaml;

    // ECH 启用时，处理代理节点添加 ech-opts
    const lines = clash_yaml.split('\n');
    const processedLines = [];
    let i = 0;
    let nodeCount = 0; // 追踪已处理节点数，每12个节点yield一次
    ...
    while (i < lines.length) {
        ...
        if (trimmedLine.startsWith('- {') && (trimmedLine.includes('uuid:') || trimmedLine.includes('password:'))) {
            nodeCount++;
            if (nodeCount % 12 === 0) await yieldToEventLoop();
            let fullNode = line;
            ...
            if (credentialMatch && ECH启用 && credentialMatch[1].trim() === uuid.trim()) {
...
async function Singbox订阅配置文件热补丁(SingBox_原始订阅内容, uuid = null, fingerprint = "chrome", ECH启用 = false, HOSTS = [], ECH_SNI = null, ECH_DNS = null) {
    ...
    try {
        ...
        // yield 后再执行 JSON.stringify，避免同步大对象序列化超 CPU 时间片
        await yieldToEventLoop();
        return JSON.stringify(config);
    } catch (e) {
        console.error("Singbox热补丁执行失败:", e);
        // 注意：catch 内 sb_json_text 可能为无效 JSON，需兜底防止二次抛异常
        try {
            return JSON.stringify(JSON.parse(sb_json_text));
        } catch {
            return SingBox_原始订阅内容; // 兜底：返回原始内容
        }
    }
}
...
async function 请求日志记录(env, request, 访问IP, 请求类型 = "Get_SUB", config_JSON, 是否写入KV日志 = true) {
    try {
        ...
        // [隐私增强] IP 哈希化（默认开启，设 false/0 关闭）
        const LOG_ANONYMIZE = env.LOG_ANONYMIZE ? !['0', 'false'].includes(env.LOG_ANONYMIZE) : true;
        let 日志IP = 访问IP;
        if (LOG_ANONYMIZE && 访问IP) {
            const salt = env.LOG_IP_SALT || 特征码字典[1].slice(0,3).toUpperCase() + 特征码字典[1].slice(3) + 'ssss' + '_log_default_salt';
            try {
                const hash = hexEncode(await crypto.subtle.digest('SHA-256', SS文本编码器.encode(访问IP + salt)));
                日志IP = hash.slice(0, 12);
            } catch (e) { /* 哈希失败则保留原值 */ }
        }
        // [隐私增强] 地区精度降级（默认开启，设 false/0 关闭）
        const 缩略地区 = env.LOG_COMPACT_REGION ? !['0', 'false'].includes(env.LOG_COMPACT_REGION) : true;
        const 日志CC = 缩略地区
            ? (request.cf?.country || 'N/A')
            : `${request.cf?.country || 'N/A'} ${request.cf?.city || 'N/A'}`;
        // [隐私增强] ASN 组织名称默认隐藏（设 LOG_ASN_ORG=true 显示），防止缩小定位范围
        const 显示ASN组织 = ['1', 'true'].includes(env.LOG_ASN_ORG);
        const ASN编号 = `AS${request.cf?.asn || '0'}`;
        const ASN文本 = 显示ASN组织 ? `${ASN编号} ${request.cf?.asOrganization || 'Unknown'}` : ASN编号;
        const 日志内容 = { TYPE: 请求类型, IP: 日志IP, ASN: ASN文本, CC: 日志CC, URL: sanitizeURL(request.url), UA: sanitizedUA, TIME: 当前时间.getTime() };
        if (config_JSON.TG.启用) {
            try {
                if (!tgJSONCache || Date.now() - tgJSONCacheTime > 300000) {
                    const TG_TXT_RAW = await withTimeout(env.KV.get('tg.json'), 3000);
                    const TG_TXT = await decryptKV(TG_TXT_RAW, env);
        ...
        const KV日志flush条数 = Number(env.KV_LOG_FLUSH_MAX) || 50;
        const KV日志flush间隔 = Number(env.KV_LOG_FLUSH_INTERVAL_MS) || 300000; // 默认5分钟
        if (globalLogQueue.length >= KV日志flush条数 || (globalLogQueue.length > 0 && Date.now() - lastLogFlushTime > KV日志flush间隔)) {
            ...
            try {
                ...
                const KV日志条数限制 = 1000;
                // [隐私增强] 日志自动过期裁剪（默认保留30天，设 0 关闭）
                const 日志最大保留天数 = Number(env.LOG_RETENTION_DAYS) || 30;
                const 过期阈值 = 日志最大保留天数 > 0 ? Date.now() - 日志最大保留天数 * 86400000 : 0;
                let 日志数组 = [];
                if (现有日志) {
                    try {
                        日志数组 = JSON.parse(现有日志);
                        if (!Array.isArray(日志数组)) 日志数组 = [];
                        if (过期阈值 > 0) 日志数组 = 日志数组.filter(entry => entry.TIME >= 过期阈值);
                    } catch (e) { 日志数组 = []; }
                }
...
async function sendMessage(BotToken, ChatID, 日志内容, config_JSON) {
    ...
    try {
        ...
        // [隐私增强] 改用 POST 发送，避免消息内容和 BotToken 暴露在 URL 中
        const tgApiUrl = `https://api.telegram.org/bot${BotToken}/sendMessage`;
        return fetchWithTimeout(tgApiUrl, {
            method: 'POST',
            headers: {
...
// 纯 JS MD5 实现（无依赖；兼容 Node.js WebCrypto 环境，crypto.subtle.digest 不支持 MD5）
function MD5纯JS(输入) {
    ...
    const 数据 = 输入 instanceof Uint8Array ? 输入 : new Uint8Array(输入);
    const 位长度 = 数据.length * 8;
    const 填充长度 = (((数据.length + 8) >> 6) + 1) << 6;
    const 填充 = new Uint8Array(填充长度);
    填充.set(数据);
    填充[数据.length] = 0x80;
    const 视图 = new DataView(填充.buffer);
    for (let i = 0; i < 8; i++) 视图.setUint8(填充长度 - 8 + i, (位长度 / Math.pow(2, 8 * i)) & 0xff);
    let a0 = 0x67452301, b0 = 0xefcdab89, c0 = 0x98badcfe, d0 = 0x10325476;
    for (let 块起点 = 0; 块起点 < 填充长度; 块起点 += 64) {
        const M = new Uint32Array(16);
        for (let i = 0; i < 16; i++) M[i] = 视图.getUint32(块起点 + i * 4, true);
    ...
    new DataView(结果.buffer).setUint32(0, a0, true);
    new DataView(结果.buffer).setUint32(4, b0, true);
    new DataView(结果.buffer).setUint32(8, c0, true);
    new DataView(结果.buffer).setUint32(12, d0, true);
    return 结果;
}
...
async function MD5MD5(文本) {
    ...
    const 第一次十六进制 = hexEncode(MD5纯JS(SS文本编码器.encode(文本)));
    const result = hexEncode(MD5纯JS(SS文本编码器.encode(第一次十六进制.slice(7, 27)))).toLowerCase();
...
async function AESEncrypt(plaintext, password) {
    ...
    try {
        ...
        if (!keyBytes) {
            ...
            keyBytes = MD5纯JS(SS文本编码器.encode(password)); // MD5 = 16 字节，恰好 AES-128 密钥长度
...
// AES 解密函数（与 AESEncrypt 对称）
async function AESDecrypt(cipherBase64, password) {
    if (!cipherBase64 || !password) return null;
    try {
        const raw = uint8ArrayFromBase64(cipherBase64);
        const iv = raw.slice(0, 16);
        const data = raw.slice(16);
        const md5 = MD5纯JS(SS文本编码器.encode(password));
        const key = await crypto.subtle.importKey('raw', md5, { name: 'AES-CBC', length: 128 }, false, ['decrypt']);
        const decrypted = await crypto.subtle.decrypt({ name: 'AES-CBC', iv }, key, data);
        return $decoder.decode(decrypted);
    } catch (e) { console.error('AES解密失败:', e); return null; }
}

// Base64 → Uint8Array 解码（与 uint8ArrayToBase64 对称）
function uint8ArrayFromBase64(str) {
...
// KV 凭据加密/解密包装（无 ENCRYPT_KEY 时透传）
async function encryptKV(plaintext, env) {
    const key = env?.ENCRYPT_KEY || null;
    if (!key || !plaintext) return plaintext;
    return AESEncrypt(plaintext, key);
}
async function decryptKV(ciphertext, env) {
    const key = env?.ENCRYPT_KEY || null;
...
/**
 * 稳健抽取 ECH_SNI：config_JSON.HOSTS → env.HOST → fallbackHost，避免 KV 加密迁移时 HOSTS 为空导致崩溃
 */
function 获取ECH_SNI(config_JSON, fallbackHost) {
    let hosts = config_JSON?.HOSTS;
    if (!hosts || !hosts.length) {
        // config_JSON.HOSTS 不可用时顺位 fallback
        hosts = fallbackHost ? [String(fallbackHost).toLowerCase().replace(/^https?:\/\//, '').split('/')[0].split(':')[0]] : ['proxy'];
    }
...
async function DoH查询(域名, 记录类型, DoH解析服务 = "https://cloudflare-dns.com/dns-query") {
    ...
    const qtype = 类型映射[规范化记录类型] || 1;
    const 当前时间戳 = Date.now();
    const 现缓存项 = DoH函数缓存[缓存键];
    ...
    try {
        ...
        // === 写入函数级缓存（参考上游 77c2097） ===
        // 最小 TTL 5分钟，空结果也缓存（负缓存），避免反复查询不存在的域名
        ...
        if (缓存数据.length > 0 || answers.length === 0) {
            // 缓存满时先清理过期项
            if (Object.keys(DoH函数缓存).length >= DoH函数缓存最大条目) {
                const 清理时间戳 = Date.now();
                for (const [缓存条目键, 缓存条目] of Object.entries(DoH函数缓存)) {
                    if (清理时间戳 >= 缓存条目.过期时间) delete DoH函数缓存[缓存条目键];
                }
                // 仍满则删最旧一条
                if (Object.keys(DoH函数缓存).length >= DoH函数缓存最大条目) {
                    delete DoH函数缓存[Object.keys(DoH函数缓存)[0]];
                }
            }
            DoH函数缓存[缓存键] = { data: 缓存数据, 过期时间: 缓存过期时间 };
            log(`[DoH查询] 写入缓存 ${域名} ${记录类型} TTL=${缓存TTL}s${缓存数据.length === 0 ? '（空结果）' : ''}`);
        }
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false, existingToken = null) {
    ...
    const _p = 特征码字典[0];
    const host = hostname, CM_DoH = `https://doh.${特征码字典[1]}ssss.net/${特征码字典[1].slice(0,3).toUpperCase() + 特征码字典[1].slice(3) + 'ssss'}`, 占位符 = '{{IP:PORT}}', 初始化开始时间 = performance.now(), 默认配置JSON = {
        TIME: new Date().toISOString(),
        HOST: host,
        HOSTS: [hostname],
        UUID: userID,
...
[_p]: 特征码字典[0].toLowerCase() + "=" + 占位符,
    ...
    if (configRaw === KV_TIMEOUT) {
        ...
        try {
            await withTimeout(env.KV.put('config.json', await encryptKV(JSON.stringify(默认配置JSON, null, 2), env)), 3000);
        } catch (e) {
    ...
    // ★ env.TLS_PADDING 覆盖 TLS填充 开关（遵循代码库布尔 env 模式）
    if (env.TLS_PADDING !== undefined) {
        config_JSON.TLS填充 = ['1', 'true'].includes(env.TLS_PADDING);
    }
    const TLS分片参数 = 获取TLS分片参数(config_JSON);
    ...
    if (!config_JSON.SNIMixing) {
        config_JSON.SNIMixing = { 启用: false, 真实SNI比例范围: [0.71, 0.81], 白名单: 默认SNI白名单 };
    }
    ...
    if (env.SNIMIXING) {
        try {
            ...
            // 兼容旧字段名: 真实SNI比例 → 真实SNI比例范围（标量 → 数组）
            if (envSNI.真实SNI比例 != null && envSNI.真实SNI比例范围 == null) {
                envSNI.真实SNI比例范围 = Array.isArray(envSNI.真实SNI比例)
                    ? envSNI.真实SNI比例
                    : [envSNI.真实SNI比例, envSNI.真实SNI比例];
            }
    ...
    const ECH_SNI = 获取ECH_SNI(config_JSON, hostname);
    const ECHLINK参数 = config_JSON.ECH ? `&ech=${encodeURIComponent(ECH_SNI + '+' + config_JSON.ECHConfig.DNS)}` : '';
    ...
    config_JSON.LINK = config_JSON.协议类型 === 'ss'
        ? `${config_JSON.协议类型}://${btoa(config_JSON.SS.加密方式 + ':' + userID)}@${host}:${config_JSON.SS.TLS ? '443' : '80'}?plugin=v2${encodeURIComponent(`ray-plugin;mode=websocket;host=${host};path=${((config_JSON.完整节点路径.includes('?') ? config_JSON.完整节点路径.replace('?', '?enc=' + config_JSON.SS.加密方式 + '&') : (config_JSON.完整节点路径 + '?enc=' + config_JSON.SS.加密方式)) + (config_JSON.SS.TLS ? ';tls' : ''))};mux=false`) + SS_ECH参数 + TLS分片参数}#${encodeURIComponent(config_JSON.优选订阅生成.SUBNAME)}`
        : `${config_JSON.协议类型}://${userID}@${host}:443?security=tls&type=${LINK传输协议 + ECHLINK参数}&${LINK域名字段名}=${host}&fp=${config_JSON.Fingerprint}&sni=${host}&${LINK路径字段名}=${encodeURIComponent(config_JSON.随机路径 ? 随机路径(config_JSON.完整节点路径) : config_JSON.完整节点路径) + TLS分片参数}&encryption=none&alpn=${encodeURIComponent(config_JSON.ALPN)}${config_JSON.跳过证书验证 ? '&insecure=1&allowInsecure=1' : ''}#${encodeURIComponent(config_JSON.优选订阅生成.SUBNAME)}`;
    ...
    if (tgRaw === KV_TIMEOUT) {
        ...
        try {
            await withTimeout(env.KV.put('tg.json', await encryptKV(JSON.stringify(初始化TG_JSON, null, 2), env)), 3000);
        } catch (e) {
    ...
    if (cfRaw === KV_TIMEOUT) {
        ...
        try {
            await withTimeout(env.KV.put('cf.json', await encryptKV(JSON.stringify(初始化CF_JSON, null, 2), env)), 3000);
        } catch (e) {
...
async function 请求优选API(urls, 默认端口 = '443', 超时时间 = 8000) {
    ...
    const fetchWithRetry = async (url, maxRetries = 2) => {
        ...
        for (let i = 0; i < maxRetries; i++) {
            try {
                const controller = new AbortController();
                const timeoutId = setTimeout(() => controller.abort(), 超时时间);
                const response = await fetch(url, { signal: controller.signal, headers: { 'User-Agent': 汇聚订阅_UA } });
                clearTimeout(timeoutId);
    ...
    await Promise.allSettled(urls.map(async (url) => {
        ...
        const 优选IP作为反代IP = url.toLowerCase().includes(特征码字典[0].toLowerCase() + '=true');
...
// === Trojan Fallback（参考上游 338326c 思路，按需启用） ===
// 用法：URL 路径中加入 /trojan=IP:PORT，当 Trojan 协议连接时使用该地址作为回退
// 适用场景：自建 Trojan 入口对接，fallback 服务需同密码、非 WebSocket、非 TLS
function 解析木马反代地址(address) {
    ...
    if (raw.startsWith('[')) {
        ...
        hostname = 匹配[1]; portText = 匹配[2];
    } else {
        const parts = raw.split(':');
        if (parts.length !== 2) throw new Error('木马反代仅支持 host:port');
        hostname = parts[0]; portText = parts[1];
    }
...
// 连接木马反代：TCP 连接到 fallback 地址并发送首包数据
async function 连接木马反代(data, TCP连接, 木马反代地址) {
    if (!木马反代地址) throw new Error('trojan fallback is not configured');
    const socket = TCP连接({ hostname: stripIPv6Brackets(木马反代地址.hostname), port: 木马反代地址.port });
    let writer = null;
    try {
...
// 提取木马反代握手数据：仅返回 Trojan header（不含 payload）用于后续重连
// 首次连接发送完整数据（header + payload），后续重连只发 header，
// 因为 payload 已从流中消费，connectStreams 会转发后续包
...
// 转发木马UDP反代数据：通过 Trojan fallback 转发 UDP 数据
...
async function 反代参数获取(url, uuid, ctx2) {
    ...
    // 从 config_JSON 加载默认代理配置（如果 URL 未覆盖）
    if (!ctx2.我的SOCKS5账号 && config_JSON?.反代?.SOCKS5?.账号) {
        ctx2.启用SOCKS5反代 = config_JSON.反代.SOCKS5.启用;
        ctx2.启用SOCKS5全局反代 = config_JSON.反代.SOCKS5.全局;
        ctx2.我的SOCKS5账号 = config_JSON.反代.SOCKS5.账号;
    }
    ...
    if (链式代理路径匹配) {
        try {
            const 链式代理明文 = await decodeChainProxyData(链式代理路径匹配[1].replace(/\/+$/, ''), uuid);
            ...
            if (isNaN(ctx2.parsedSocks5Address.port)) throw new Error('链式代理端口无效');
            return;
        } catch (err) {
    ...
    // Trojan Fallback 路径解析：/trojan=IP:PORT（参考上游 338326c）
    const 木马路径匹配 = /\/trojan=([^?#\s]+)/i.exec(pathname);
    if (木马路径匹配) {
        try {
            ctx2.木马反代地址 = 解析木马反代地址(木马路径匹配[1].replace(/\/+$/, ''));
            log(`[Trojan Fallback] 回退地址: ${ctx2.木马反代地址.hostname}:${ctx2.木马反代地址.port}`);
        } catch (err) {
    ...
    if (searchParams.has(特征码字典[0].toLowerCase())) {
        const 路参IP = searchParams.get(特征码字典[0].toLowerCase());
    ...
    else if (proxyMatch = pathLower.match(new RegExp('\\/(' + 特征码字典[0].toLowerCase() + '[.=]|pyip=|ip=)([^?#\\s]+)'))) {
...
async function 获取SOCKS5账号(address, 默认端口 = 80) {
    ...
    const [hostPart, authPart] = atIndex === -1 ? [address.split('/')[0], undefined] : [address.substring(atIndex + 1).split('/')[0], address.substring(0, atIndex)];
...
async function SOCKS5可用性验证(代理协议 = 'socks5', 代理参数, request = null) {
    const startTime = Date.now();
    let socks5Addr;
    try { socks5Addr = await 获取SOCKS5账号(代理参数, 获取代理默认端口(代理协议)); } catch (err) { return { success: false, error: err.message, proxy: 代理协议 + "://" + 代理参数, responseTime: Date.now() - startTime }; }
    const { username, password, hostname, port } = socks5Addr;
    const 完整代理参数 = username && password ? `${username}:${password}@${hostname}:${port}` : `${hostname}:${port}`;
    try {
        ...
        if (代理协议 === 'socks5') {
            tcpSocket = await socks5Connect(`check.socks5.${特征码字典[2]}.xyz`, 80, initialData, TCP连接, socks5Addr);
        } else if (代理协议 === 'turn') {
            tcpSocket = await turnConnect(socks5Addr, `check.socks5.${特征码字典[2]}.xyz`, 80, TCP连接);
        } else if (代理协议 === 'sstp') {
            tcpSocket = await sstpConnect(socks5Addr, `check.socks5.${特征码字典[2]}.xyz`, 80, TCP连接);
        } else {
            tcpSocket = 代理协议 === 'https'
                ? await httpsConnect(`check.socks5.${特征码字典[2]}.xyz`, 80, initialData, TCP连接, socks5Addr)
                : await httpConnect(`check.socks5.${特征码字典[2]}.xyz`, 80, initialData, TCP连接, socks5Addr);
        }
