# 合并冲突报告
## 冲突时间: Wed Jun 10 03:47:51 UTC 2026
## 上游更新哈希: fc768b1b90e5e70df03860892771f484260c58a6a137825b109dfb28a291de2e
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (env.PROXYIP) {
            ...
            ctx2.反代IP = (`${request.cf?.colo || 'unknown'}.${查杀特征码}.CmLiUsSsS.nEt`).toLowerCase();
            ctx2.启用反代兜底 = true;
        }
                ...
                if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
                    ...
                    if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
                        ...
                        if (['mixed', 'base64', 'v2ray'].includes(订阅类型)) {
                            const TLS分片参数 = config_JSON.TLS分片 == 'Shadowrocket' ? `&fragment=${encodeURIComponent('0-5,500-1500,100-200,tlshello')}` : config_JSON.TLS分片 == 'Happ' ? `&fragment=${encodeURIComponent('500-1500,0-5,tlshello,100-200')}` : config_JSON.TLS填充 ? `&padding=${encodeURIComponent('0-128')}` : '';
                            ...
                            const 订阅转换URL = `${config_JSON.订阅转换配置.SUBAPI}/sub?target=${订阅类型}&url=${encodeURIComponent(url.protocol + '//' + url.host + '/sub?target=mixed&token=' + 今日订阅转换后端专属TOKEN + cnIspCodeParam + (url.searchParams.has('sub') && url.searchParams.get('sub') != '' ? `&sub=${url.searchParams.get('sub')}` : '') + (queryAF ? `&af=${encodeURIComponent(queryAF)}` : '') + needsAF)}&config=${encodeURIComponent(config_JSON.订阅转换配置.SUBCONFIG)}&emoji=${config_JSON.订阅转换配置.SUBEMOJI}&scv=${config_JSON.跳过证书验证}`;
                            try {
                                const response = await fetch(订阅转换URL, { headers: { 'User-Agent': 'Subconverter for ' + 订阅类型 + ' edge' + 'tunnel (https://github.com/cmliu/edge' + 'tunnel)' } });
                                if (response.ok) {
...
async function forwardataTCP(host, portNum, rawData, ws, respHeader, remoteConnWrapper, yourUUID, ctx2) {
	...
	const log = (...args) => ctx2.log(...args);
    log(`[TCP转发] 目标: ${sanitizeHostname(host)}:${portNum} | 反代IP: ${反代IP} | 反代兜底: ${启用反代兜底 ? '是' : '否'} | 反代类型: ${启用SOCKS5反代 || 'proxyip'} | 全局: ${启用SOCKS5全局反代 ? '是' : '否'}`);
    const 连接超时毫秒 = 5000;
    let 已通过代理发送首包 = false;
...
async function 请求日志记录(env, request, 访问IP, 请求类型 = "Get_SUB", config_JSON, 是否写入KV日志 = true) {
    try {
        ...
        if (LOG_ANONYMIZE && 访问IP) {
            const salt = env.LOG_IP_SALT || 'edgetunnel_log_default_salt';
            try {
...
async function SOCKS5可用性验证(代理协议 = 'socks5', 代理参数, request = null) {
    ...
    try {
        ...
        if (代理协议 === 'socks5') {
            tcpSocket = await socks5Connect('check.socks5.090227.xyz', 80, initialData, TCP连接, socks5Addr);
        } else if (代理协议 === 'turn') {
            tcpSocket = await turnConnect(socks5Addr, 'check.socks5.090227.xyz', 80, TCP连接);
        } else if (代理协议 === 'sstp') {
            tcpSocket = await sstpConnect(socks5Addr, 'check.socks5.090227.xyz', 80, TCP连接);
        } else {
            tcpSocket = 代理协议 === 'https'
                ? await httpsConnect('check.socks5.090227.xyz', 80, initialData, TCP连接, socks5Addr)
                : await httpConnect('check.socks5.090227.xyz', 80, initialData, TCP连接, socks5Addr);
        }
