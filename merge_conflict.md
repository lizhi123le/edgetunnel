# 合并冲突报告
## 冲突时间: Fri Apr 17 13:00:53 UTC 2026
## 上游更新哈希: cf48abaa1b82d2a4b4f66b20424efb7ae955c72b387f44d47af48e1529c29779

以下文件包含冲突标记，需要手动解决：

```
const Version = '2026-04-16 04:47:24 (Local Enhanced)';
...
<<<<<<< local_worker.js
    async fetch(request, env, ctx) {
        ...
        if (env.GO2SOCKS5) SOCKS5白名单 = ctx2.SOCKS5白名单 = await 整理成数组(env.GO2SOCKS5);
...
async function hkdfExpandLabel(hash, secret, label, context, length) {
	const fullLabel = TLS文本编码er.encode('tls13 ' + label);
...
class TlsClient {
	...
	async decryptTls13Handshake(ciphertext) {
		...
		if (!this.serverHandshakeCryptoKey) this.serverHandshakeCryptoKey = await TLS导入AES密钥(this.serverHandshakeKey, ['decrypt']);
		return TLS_AES_GCM解密(this.serverHandshakeCryptoKey, nonce, ciphertext, additionalData);
	}
	...
	async decryptTls13(ciphertext) {
		...
		return { data: plaintext.subarray(0, plaintext.length - 1), type: plaintext[plaintext.length - 1] };
	}
        ...
        if (uuid) {
            for (const outbound of config.outbounds) {
                ...
                if ((outbound.uuid && outbound.uuid === uuid) || (outbound.password && outbound.password === uuid)) {
                    ...
                    // 如果提供了 ECH启用，添加/更新 ech 配置
                    if (ECH启用) {
                        const nodeSNI = (ECH_SNI && ECH_SNI !== 'auto') ? ECH_SNI : 随机替换通配符(HOSTS[Math.floor(Math.random() * HOSTS.length)]);
                        const nodeEchConfig = await getECH(nodeSNI);
                        if (nodeEchConfig) {
                            outbound.tls.ech = {
                                enabled: true,
