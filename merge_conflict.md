# 合并冲突报告
## 冲突时间: Fri Apr  3 23:51:41 UTC 2026
## 上游更新哈希: 84376c11779b9b3ec7267f997e1be5538614fc4bfe1fa54513d40708bc9d248a
...
// 修正SB内核请求URL函数 - 处理SB内核发送的URL编码问题
                ...
                if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
                    ...
                    if (url.searchParams.get('token') === 订阅TOKEN || 作为优选订阅生成器) {
                            ...
                            if (!url.searchParams.has('sub') && config_JSON.优选订阅生成.local) { // 本地生成订阅
                                ...
                                const IP转换节点 = (原始地址) => {
                                    ...
                                    if (协议类型 === 'ss') {
                                        ...
                                        return `${协议类型}://${btoa(config_JSON.SS.加密方式 + ':' + config_JSON.UUID)}@${节点地址}:${节点端口}?plugin=v2${encodeURIComponent('ray-plugin;mode=websocket;host=example.com;path=' + 完整节点路径 + (config_JSON.SS.TLS ? ';tls' : '')) + ECHLINK参数}#${encodeURIComponent(节点备注)}`;
                                    } else return `${协议类型}://${config_JSON.UUID}@${节点地址}:${节点端口}?security=tls&type=${传输协议 + ECHLINK参数}&${域名字段名}=example.com&fp=${config_JSON.Fingerprint}&sni=example.com&${路径字段名}=${encodeURIComponent(作为优选订阅生成器 ? '/' : (config_JSON.随机路径 ? 随机路径(完整节点路径) : 完整节点路径)) + TLS分片参数}&encryption=none${config_JSON.跳过证书验证 ? '&insecure=1&allowInsecure=1' : ''}#${encodeURIComponent(节点备注)}`;
                                };

                                for (const 元素 of 完整优选列表) {
                                    if (元素.toLowerCase().startsWith('sub://') || 元素.toLowerCase().startsWith('https://') || (元素.match(/sub\s*=\s*([^\s&#]+)/i) && 元素.match(/sub\s*=\s*([^\s&#]+)/i)[1].trim().includes('.'))) {
                                        let key = 元素;
                                        if (!元素.toLowerCase().startsWith('https://') && !元素.toLowerCase().startsWith('sub://')) {
                                            const subMatch = 元素.match(/sub\s*=\s*([^\s&#]+)/i);
                                            const 优选IP作为反代IP = 元素.toLowerCase().includes('proxyip=true');
                                            key = 'sub://' + subMatch[1].trim() + (优选IP作为反代IP ? "?proxyip=true" : "") + (元素.includes('#') ? ('#' + 元素.split('#')[1]) : '');
                                        }
                                        const res = resultsMap.get(key);
                                        if (res) {
                                            if (res.Nodes && res.Nodes.length > 0) 订阅列表.push(...res.Nodes);
                                            if (res.IPs && res.IPs.length > 0) 订阅列表.push(...res.IPs.map(ip => IP转换节点(ip)).filter(item => item !== null));
                                        }
                                    } else if (元素.toLowerCase().includes('://')) {
                                        if (元素.includes('#')) {
                                            const 地址备注分离 = 元素.split('#');
                                            订阅列表.push(地址备注分离[0] + '#' + encodeURIComponent(decodeURIComponent(地址备注分离[1])));
                                        } else 订阅列表.push(元素);
                                    } else {
                                        const node = IP转换节点(元素);
                                        if (node) 订阅列表.push(node);
                                    }
                                }
                            } else { // 优选订阅生成器
                        ...
                        if (订阅类型 === 'singbox') {
                            ...
                            订阅内容 = Clash订阅配置文件热补丁(订阅内容, config_JSON.UUID, config_JSON.ECH, config_JSON.HOSTS, config_JSON.ECHConfig.SNI, config_JSON.ECHConfig.DNS, config_JSON.gRPCUserAgent);
                            responseHeaders["content-type"] = 'text/plain; charset=utf-8';
                            responseHeaders["Content-Disposition"] = "inline";
                        }
...
>>>>>>> upstream_worker.js
	...
	const readable = new ReadableStream({
		start(controller) {
<<<<<<< local_worker.js
			serverSock.addEventListener('message', (e) => controller.enqueue(e.data));
			...
			} catch (err) { log(`[WS] earlyData 解码失败: ${err.message}`); }
		},
		cancel() { 已取消读取 = true; try { remoteConnWrapper.socket?.close(); } catch (e) { } }
=======
...
<<<<<<< local_worker.js
							if (!出站盐已发送) { outboundChunks.push(出站盐); 出站盐已发送 = true; }
=======
							if (!出站盐已发送) {
...
<<<<<<< local_worker.js
								outboundChunks.push(lengthCipher, payloadCipher); offset = end;
=======
...
<<<<<<< local_worker.js
							if (encrypted.byteLength > 0 && serverSock.readyState === WebSocket.OPEN) serverSock.send(encrypted.buffer);
						}).catch((error) => { log(`[SS发送] 加密失败: ${error?.message || error}`); closeSocketQuietly(serverSock); });
					},
...
function Clash订阅配置文件热补丁(Clash_原始订阅内容, uuid = null, ECH启用 = false, HOSTS = [], ECH_SNI = null, ECH_DNS, gRPCUserAgent = null) {
...
async function AESEncrypt(plaintext, password) {
    ...
    try {
        const encoder = new TextEncoder();
        const keyHash = await crypto.subtle.digest('MD5', encoder.encode(password));
