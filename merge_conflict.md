# 合并冲突报告
## 冲突时间: Wed May 27 01:11:26 UTC 2026
## 上游更新哈希: 0046b4f3212431dd9392b11203c5d293a0a13e4fce611a6bfbb35f7828358cf0

以下文件包含冲突标记，需要手动解决：

```
const Version = '2026-05-18 18:37:45(Enhanced)';
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (访问路径 === '__sni_status') {
            ...
            const stats = sni混合器 ? sni混合器.获取统计() : '未初始化';
        ...
        async start(controller) {
            ...
            try {
                if (首包.isUDP) {
                    ...
                    await forwardataTCP(首包.hostname, 首包.port, 首包.rawData, xhttpBridge, 首包.respHeader, remoteConnWrapper, yourUUID, ctx2);
                }
                        ...
                        if (remoteConnWrapper.socket) {
                            ...
                            if (判断是否是木马) {
                                ...
                                if (isUDP) {
                                    ...
                                    await forwardataTCP(hostname, port, rawClientData, grpcBridge, null, remoteConnWrapper, yourUUID, ctx2);
                                }
                            } else {
                                判断是否是木马 = false;
...
async function 处理WS请求(request, yourUUID, url, ctx2) {
	...
	const 处理SS数据 = async (chunk) => {
		...
		for (const 明文块 of 明文块数组) {
			...
			上下文.目标主机 = hostname;
			上下文.目标端口 = port;
			await forwardataTCP(hostname, port, rawClientData, 上下文.回包Socket, null, remoteConnWrapper, yourUUID, ctx2);
		}
	};
	...
	const 处理WS入站数据 = async (chunk) => {
		...
		if (判断协议类型 === '木马') {
			...
			await forwardataTCP(hostname, port, rawClientData, serverSock, null, remoteConnWrapper, yourUUID, ctx2);
		} else {
			判断是否是木马 = false;
			当前块字节 = 当前块字节 || 有效块;
			const bytes = 当前块字节;
...
function 创建SNI混合器(config = {}) {
    ...
    return { 选择SNI, 获取统计, 重置统计: () => { 混合计数 = { 主域: 0, 掩护: 0, 总计: 0 }; } };
}
...
async function forwardataTCP(host, portNum, rawData, ws, respHeader, remoteConnWrapper, yourUUID, ctx2) {
	const { 反代IP, 启用反代兜底, 启用SOCKS5反代, 启用SOCKS5全局反代, parsedSocks5Address, SOCKS5白名单正则, 调试日志打印 } = ctx2;
	const log = (...args) => ctx2.log(...args);
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false, existingToken = null) {
    ...
    // 3. 创建全局混合器实例
    全局SNI混合器 = 创建SNI混合器(config_JSON.SNIMixing);
    const ECH_SNI = 随机替换通配符(config_JSON.HOSTS[Math.floor(Math.random() * config_JSON.HOSTS.length)]);
    const ECHLINK参数 = config_JSON.ECH ? `&ech=${encodeURIComponent(ECH_SNI + '+' + config_JSON.ECHConfig.DNS)}` : '';
