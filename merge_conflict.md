# 合并冲突报告
## 冲突时间: Fri May 29 01:11:42 UTC 2026
## 上游更新哈希: d2e95d242434ef9becfd1ef892bfdcd385f2ab75242700f4d1b62ca33e63629c
...
function createYieldStrategy(intervalMs = 3) {
    let lastYield = 0;
...
function sanitizedErrorResponse(message, status = 500) {
    ...
    return new Response(JSON.stringify({ error: publicMessages[status] || '操作失败' }), {
        status,
                ...
                if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
                    ...
                    if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
                        ...
                        if (['mixed', 'base64', 'v2ray'].includes(订阅类型)) {
                            const TLS分片参数 = config_JSON.TLS分片 == 'Shadowrocket' ? `&fragment=${encodeURIComponent('1200-1500,1200-1500,0-5,tlshello')}` : config_JSON.TLS分片 == 'Happ' ? `&fragment=${encodeURIComponent('1200-1500,1200-1500,0-5,tlshello')}` : config_JSON.TLS填充 ? `&padding=${encodeURIComponent('0-32')}` : '';
                        ...
                        ctx.waitUntil(cache.put(cacheKey, subResponse.clone()));
                        return subResponse;
                    }
...
async function 处理WS请求(request, yourUUID, url, ctx2) {
	...
	let wsAccept = '';
	if (wsKey) {
		const wsHash = await crypto.subtle.digest('SHA-1', new TextEncoder().encode(wsKey + WS_GUID));
		wsAccept = btoa(String.fromCharCode(...new Uint8Array(wsHash)));
	}
	const wsRespHeaders = {
		'Upgrade': 'websocket',
		'Connection': 'Upgrade',
...
const SSAEAD标签长度 = 16;
const SSNonce长度 = 12;
const SS子密钥信息 = $encoder.encode('ss-subkey');
...
function 尝试改写ClientHelloSNI(rawData, sni混合器) {
    ...
    while (offset + 4 <= extEnd) {
        ...
        if (extType === 0x0000) {
            ...
            const originalSNI = new TextDecoder().decode(sniBytes);
            
            // 调用 SNI 混合器选择新的 SNI
            const newSNI = sni混合器.选择SNI(originalSNI);
            if (!newSNI || newSNI === originalSNI) return null;  // 无需改写
            
            const newSNIBytes = new TextEncoder().encode(newSNI);
...
async function ChaCha20XOR(key, nonce, data) {
        ...
        const poly1305Yield = createYieldStrategy(3);
...
// ===== SNI 混合模块（服务器端真随机分发）=====
const 默认SNI白名单 = [
    'www.iqiyi.com',      // 爱奇艺 CDN 自研架构，节点超 2000，SNI 替换失败率 < 2%
    'm.iqiyi.com',
    'www.youku.com',      // 阿里视频生态，HTTP/2 默认，路由调度宽容
    'm.youku.com',
    'www.mgtv.com',       // 芒果 TV，多活容灾强，TLS 指纹波动低
    'm.mgtv.com',
    'www.bilibili.com',   // 二次元社区，CDN 覆盖广，DPI 识别为常规流媒体
    'www.acfun.cn',
    'www.pptv.com',       // 传统视频平台，架构稳定，证书轮换周期长
    'www.fun.tv',
];
...
function 确保SNI混合器已初始化(env) {
    if (全局SNI混合器) return;
    const config = { 启用: false, 真实SNI比例: 0.8, 白名单: 默认SNI白名单 };
    if (env.SNIMIXING) {
        try {
        ...
        try {
            ...
            if (this.echEnabled && this.echConfig) {
                try {
                    ...
                    await writer.write(buildTlsRecord(CONTENT_TYPE_HANDSHAKE, echResult.outerCH, TLS_VERSION_10));
                    // ★ ECH 统计 (Step H2)
                    ...
                    const ch = buildClientHello(this.clientRandom, this.发送SNI, this.keyShares, { tls13: this.supportTls13, tls12: this.supportTls12, alpn: this.alpnProtocols, chacha: this.allowChacha });
                    this.recordHandshake(ch);
                    await writer.write(buildTlsRecord(CONTENT_TYPE_HANDSHAKE, ch, TLS_VERSION_10));
                }
            } else {
                const ch = buildClientHello(this.clientRandom, this.发送SNI, this.keyShares, { tls13: this.supportTls13, tls12: this.supportTls12, alpn: this.alpnProtocols, chacha: this.allowChacha });
                this.recordHandshake(ch);
...
function HPKE构建SuiteID() {
	return concatBytes($encoder.encode('HPKE-v1'), uint16be(HPKE_KEM_ID), uint16be(HPKE_KDF_ID), uint16be(HPKE_AEAD_ID));
}

async function HPKE标签化Extract(salt, ikm, label, suiteID) {
	const labeledIKM = concatBytes($encoder.encode('HPKE-v1'), suiteID, $encoder.encode(label), ikm);
	return hkdfExtract('SHA-256', salt, labeledIKM);
}

async function HPKE标签化Expand(prk, label, info, length, suiteID) {
	const labeledInfo = concatBytes(uint16be(length), $encoder.encode('HPKE-v1'), suiteID, $encoder.encode(label), info);
...
async function HPKE_KeySchedule(mode, sharedSecret, hpkeInfo, psk = '', psk_id = '') {
	...
	const secret = await HPKE标签化Extract(sharedSecret, $encoder.encode(''), 'secret', suiteID);
...
function 构建带ECH扩展的外层CH(clientRandom, serverName, keyShares, options, echExt) {
	const cipherIds = [];
	if (options?.tls13 !== false) cipherIds.push(4865, 4866);
	if (options?.tls13 !== false && options?.chacha !== false) cipherIds.push(4867);
	if (options?.tls12 !== false) cipherIds.push(49199, 49200, 49195, 49196);
	if (options?.tls12 !== false && options?.chacha !== false) cipherIds.push(52392, 52393);
	...
	if (options?.alpn) {
		const alpnBytes = TLS文本编码器.encode(options.alpn);
	...
	const clientHello = tlsBytes(TLS_VERSION_12, clientRandom, tlsBytes(0), cipherBytes, tlsBytes(1, 0), tlsBytes(uint16be(concatBytes(...extensions).length), ...extensions));
	return buildHandshakeMessage(HANDSHAKE_TYPE_CLIENT_HELLO, clientHello);
}

// 内层 CH padding (RFC 7685 type=21)
function addPaddingToClientHello(clientHelloMsg, targetSize) {
	const currentSize = clientHelloMsg.length;
	const paddingNeeded = targetSize - currentSize;
	if (paddingNeeded <= 4) return clientHelloMsg;
	const paddingDataLen = paddingNeeded - 4;
	const paddingExt = concatBytes(uint16be(21), uint16be(paddingDataLen), new Uint8Array(paddingDataLen));
	const body = clientHelloMsg.slice(4);
	let offset = 34;
	offset += 1 + body[offset];
	offset += 2 + ((body[offset] << 8) | body[offset + 1]);
	offset += 1 + body[offset];
	const extLen = (body[offset] << 8) | body[offset + 1];
	offset += 2;
	const existingExt = body.slice(offset, offset + extLen);
	const newExt = concatBytes(existingExt, paddingExt);
	const newBody = concatBytes(body.slice(0, offset - 2), uint16be(newExt.length), newExt);
	return buildHandshakeMessage(HANDSHAKE_TYPE_CLIENT_HELLO, newBody);
}
...
async function buildECHClientHello(clientRandom, 真实SNI, publicName, keyShares, options, echConfig) {
	let innerCH = buildClientHello(clientRandom, 真实SNI, keyShares, options);
	innerCH = addPaddingToClientHello(innerCH, 512);
	const outerCH_Base = buildClientHello(clientRandom, publicName, keyShares, options);
	const echLabel = $encoder.encode('tls13-ech');
...
async function forwardataTCP(host, portNum, rawData, ws, respHeader, remoteConnWrapper, yourUUID, ctx2) {
    ...
    const 连接超时毫秒 = 2000;
    let 已通过代理发送首包 = false;
    const TCP连接 = ctx2.request ? 创建请求TCP连接器(ctx2.request) : connect;
...
function 创建下行Grain发送器(webSocket, headerData = null) {
	...
	// ★ 隐私增强：分片大小集中于 MTU/MSS 区间 (1200-1500)，避免小分片极端直方图峰值
	const HTTPS响应大小分布 = [
		{ 最小: 1200, 最大: 1500, 权重: 0.85 }, // MTU/MSS 主流区间
		{ 最小: 1500, 最大: 3000, 权重: 0.10 }, // 偶尔 TCP 合并
		{ 最小: 3000, 最大: 8000, 权重: 0.05 }, // 罕见大包
	];
	const 获取下一个分片大小 = () => {
		...
		for (const c of HTTPS响应大小分布) {
			累计 += c.权重;
			if (rand <= 累计) return c.最小 + Math.floor(Math.random() * (c.最大 - c.最小));
		}
		return 1400;
	};
	...
	const scheduleFlush = () => {
		...
		queueMicrotask(() => {
			...
			const 目标大小 = 首包已发送 ? 获取下一个分片大小() : (200 + Math.floor(Math.random() * 200)); // 首包 200-400B
			// ★ TLS填充：小响应智能填充，添加 0-32B 等效偏移，避免精确长度指纹
			const 是否填充 = config_JSON?.TLS填充 && 首包已发送 && pendingBytes < 500;
			const 填充阈值 = 是否填充 ? 目标大小 + Math.floor(Math.random() * 33) : 目标大小;
			...
			flushTimer = setTimeout(() => {
				...
				const 填充阈值2 = 是否填充 ? 目标大小2 + Math.floor(Math.random() * 33) : 目标大小2;
	...
	return {
		...
		async 发送(data) {
			...
			while (offset < totalBytes) {
				...
				const 当前分片目标 = 首包已发送 ? 获取下一个分片大小() : (200 + Math.floor(Math.random() * 200)); // 首包 200-400B
				首包已发送 = true;
...
async function 读取config_JSON(env, hostname, userID, UA = "Mozilla/5.0", 重置配置 = false, existingToken = null) {
    ...
    const host = hostname, CM_DoH = "https://doh.cmliussss.net/CMLiussss", 占位符 = '{{IP:PORT}}', 初始化开始时间 = performance.now(), 默认配置JSON = {
        ...
        SNIMixing: {
            启用: false,              // 总开关
            真实SNI比例: 0.8,         // 0.0 - 1.0，80% 使用真实 SNI
            白名单: [                 // 掩护域名列表
                'www.iqiyi.com',
                'm.iqiyi.com',
                'www.youku.com',
    ...
    if (!config_JSON.SNIMixing) {
        config_JSON.SNIMixing = { 启用: false, 真实SNI比例: 0.8, 白名单: 默认SNI白名单 };
    }
