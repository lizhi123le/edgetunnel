const Version = '2026-04-16 (Upstream Sync + Local Enhanced)';
...
let SOCKS5白名单 = ['*tapecontent.net', '*cloudatacdn.com', '*loadshare.org', '*cdn-centaurus.com', 'scholar.google.com', 'scholar.google.com.hk', 'scholar.gstatic.com', 'scholar.googleusercontent.com'];
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					if (url.searchParams.get('token') === 订阅TOKEN || 作为优选订阅生成器) {
						...
						if (订阅类型 === 'mixed') {
							...
							const SS_ECH参数 = (config_JSON.ECH && config_JSON.SS.TLS === true && (协议类型 === 'ss' || config_JSON.协议类型 === 'ss')) ? `&echconfiglist=${encodeURIComponent((config_JSON.ECHConfig.SNI ? config_JSON.ECHConfig.SNI + '+' : '') + config_JSON.ECHConfig.DNS)}&fp=${config_JSON.Fingerprint}` : ECHLINK参数;
							...
							订阅内容 = 其他节点LINK + 完整优选IP.map(原始地址 => {
								...
								if (协议类型 === 'ss' && !作为优选订阅生成器) {
									...
									return `${协议类型}://${btoa(config_JSON.SS.加密方式 + ':00000000-0000-4000-8000-000000000000')}@${节点地址}:${节点端口}?plugin=v2${encodeURIComponent('ray-plugin;mode=websocket;host=example.com;path=' + (config_JSON.随机路径 ? 随机路径(完整节点路径) : 完整节点路径) + (config_JSON.SS.TLS ? ';tls' : '')) + SS_ECH参数 + TLS分片参数}#${encodeURIComponent(节点备注)}`;
								} else {
...
class TlsClient {
    ...
    async function Clash订阅配置文件热补丁(Clash_原始订阅内容, config_JSON = {}) {
        ...
        const ECH_SNI = (config_JSON?.ECHConfig?.SNI && config_JSON?.ECHConfig?.SNI !== 'auto')
            ? config_JSON?.ECHConfig?.SNI
            : config_JSON?.HOST || null;
        const ECH_DNS = config_JSON?.ECHConfig?.DNS;
        const gRPCUserAgent = config_JSON?.gRPCUserAgent || null;
    
        const gRPCUserAgentYAML = gRPCUserAgent ? JSON.stringify(gRPCUserAgent) : null;
        const 需要处理gRPC = Boolean(gRPCUserAgentYAML);
    
        let patchYieldTime = performance.now();
        const dynamicYield = async () => {
            if (performance.now() - patchYieldTime > 5) {
                await new Promise(r => setTimeout(r, 0));
                patchYieldTime = performance.now();
            }
        };
    
        const 添加InlineGrpcUserAgent = (text) => text.replace(
            /grpc-opts:\s*\{([\s\S]*?)\}/i,
            (all, inner) => {
        ...
        const 添加Flow格式gRPCUserAgent = (nodeText) => {
            if (!需要处理gRPC || !匹配到gRPC网络(nodeText) || /grpc-user-agent\s*:/i.test(nodeText)) {
                return nodeText;
            }
    ...
    `;
        ...
        while (i < lines.length) {
            ...
            if (trimmedLine.startsWith('- {')) {
                ...
                if (ECH启用 && credentialMatch && credentialMatch[1].trim() === uuid.trim()) {
                    const nodeSNI = (ECH_SNI && ECH_SNI !== 'auto')
                        ? ECH_SNI
                        : 随机替换通配符(HOSTS[Math.floor(Math.random() * HOSTS.length)]);
                    fullNode = fullNode.replace(
                        /\}(\s*)$/,
                        `, ech-opts: {enable: true${nodeSNI ? `, query-server-name: ${nodeSNI}` : ''}}}$1`
                    );
                }
...
async function Singbox订阅配置文件热补丁(SingBox_原始订阅内容, config_JSON = {}) {
    ...
    const ECH启用 = Boolean(config_JSON?.ECH);
    const HOSTS = Array.isArray(config_JSON?.HOSTS) ? [...config_JSON.HOSTS] : [];
    const ECH_SNI = (config_JSON?.ECHConfig?.SNI && config_JSON?.ECHConfig?.SNI !== 'auto') ? config_JSON?.ECHConfig?.SNI : config_JSON?.HOST || null;
    const ECH_DNS = config_JSON?.ECHConfig?.DNS || null;
