...
export default {
    async fetch(request, env, ctx) {
        ...
        } else 反代IP = ctx2.反代IP = (request.cf.colo + '.PrOxYIp.CmLiUsSsS.nEt').toLowerCase();
        const 访问IP = request.headers.get('CF-Connecting-IP') || request.headers.get('True-Client-IP') || request.headers.get('X-Real-IP') || request.headers.get('X-Forwarded-For') || request.headers.get('Fly-Client-IP') || request.headers.get('X-Appengine-Remote-Addr') || request.headers.get('X-Cluster-Client-IP') || '未知IP';
        if (env.GO2SOCKS5) {
            SOCKS5白名单 = ctx2.SOCKS5白名单 = await 整理成数组(env.GO2SOCKS5);
            // 同步更新预编译正则
				...
				const 入站解密器 = {
					async 输入(dataChunk) {
						...
						while (true) {
							...
							if (入站状态.cpuYieldByteCounter >= 131072) {
...
async function hkdfExtract(hash, salt, inputKeyMaterial) {
	return salt && salt.length || (salt = new Uint8Array(TLS哈希长度(hash))), TLS_HMAC(hash, salt, inputKeyMaterial);
}
async function hkdfExpandLabel(hash, secret, label, context, length) {
	...
	return async function (hash, secret, info, length) {
		...
		for (let round = 1; round <= roundCount; round++) previousBlock = await TLS_HMAC(hash, secret, concatBytes(previousBlock, info, [round])), output = concatBytes(output, previousBlock);
		return output.slice(0, length);
...
// Poly1305 MAC
        async function Poly1305Mac(key, message) {
            ...
            for (let offset = 0; offset < message.length; offset += 16) {
                if (++blockCount % 10 === 0) await new Promise(r => setTimeout(r, 0));
...
class TlsClient {
	...
	async receiveServerHello(reader) {
		...
		const preMasterSecret = await TLS派生共享密钥(keyShare.keyPair.privateKey, serverKeyExchangeData.serverPublicKey, curveName);
		...
		const finished = await this.buildTls12Finished(preMasterSecret, this.transcript());
		await writer.write(finished);
		await writer.write(buildTlsRecord(CONTENT_TYPE_CHANGE_CIPHER_SPEC));
		const [clientWriteKey, clientWriteIv, serverWriteKey, serverWriteIv] = await Promise.all([
			this.deriveTrafficKeys('SHA-256', preMasterSecret, 'client application traffic', 16, 12),
			this.deriveTrafficKeys('SHA-256', preMasterSecret, 'client application traffic', 12, 12),
			this.deriveTrafficKeys('SHA-256', preMasterSecret, 'server application traffic', 16, 12),
			this.deriveTrafficKeys('SHA-256', preMasterSecret, 'server application traffic', 12, 12)
		]);
        ...
        async handshakeTls13(reader, writer, serverHello) {
			...
			switch (message.type) {
				...
				case HANDSHAKE_TYPE_CERTIFICATE:
					this.recordHandshake(message.raw);
					await this.acceptCertificate(extractLeafCertificate(message.body));
					break;
		...
		const emptyHash = await TLS摘要('SHA-256');
		const [earlySecret, handshakeSecret, masterSecret] = await Promise.all([
			hkdfExtract('SHA-256', emptyHash),
			hkdfExtract('SHA-256', emptyHash, this.clientHandshakeKey),
			hkdfExpandLabel('SHA-256', this.handshakeSecret, 'derived', emptyHash)
		]);
		await hkdfExtract('SHA-256', masterSecret);
		const [clientAppKey, clientAppIv, serverAppKey, serverAppIv] = await Promise.all([
			this.deriveTrafficKeys('SHA-256', this.clientAppKey, 'client application traffic', 32, 12),
			this.deriveTrafficKeys('SHA-256', this.clientAppKey, 'client application traffic', 12, 12),
			this.deriveTrafficKeys('SHA-256', this.serverAppKey, 'server application traffic', 32, 12),
			this.deriveTrafficKeys('SHA-256', this.serverAppKey, 'server application traffic', 12, 12)
		]);
		this.clientAppKey = clientAppKey;
		this.clientAppIv = clientAppIv;
		this.serverAppKey = serverAppKey;
		this.serverAppIv = serverAppIv;
		this.isTls13 = true;
	}
